...
async function main() {
  ...
  for (const team of teams) {
    const { data: teamPlayers } = await supabase
      .from('jugadores')
      .select('id, nombre, apellidos, evoluciones(*), pesajes(*)')
    ...
    (teamPlayers || []).forEach(p => {
      (p.pesajes || []).forEach(pj => {
        if (pj?.fecha && pj.peso_kg !== null) {
          pesajesCount++;
          dates.add(String(pj.fecha).slice(0, 10));
        }
      });
    });
    console.log(`Equipo [${team.id}] ${team.nombre}: ${teamPlayers?.length || 0} jugadores, ${pesajesCount} pesajes, Fechas:`, Array.from(dates).sort());
  }

  // Elegir el equipo que tenga pesajes
  let valenciaTeam = teams.find((t) => t.id === 7 || t.id === 8 || t.id === 4);
  // We'll pick the one with actual pesajes
  const { data: allTeamsWithPesajes } = await supabase.from('jugadores').select('equipo_id, pesajes(id, fecha, peso_kg)');
  const teamPesajesCount = {};
  allTeamsWithPesajes.forEach(p => {
    const c = (p.pesajes || []).filter(pj => pj.peso_kg).length;
    teamPesajesCount[p.equipo_id] = (teamPesajesCount[p.equipo_id] || 0) + c;
  });
  console.log('Total pesajes por equipo:', teamPesajesCount);

  const bestTeamId = Object.entries(teamPesajesCount).sort((a, b) => b[1] - a[1])[0]?.[0];
  if (bestTeamId) {
    valenciaTeam = teams.find(t => String(t.id) === String(bestTeamId)) || valenciaTeam;
  }
  console.log(`\n==> Equipo seleccionado finalmente: [${valenciaTeam.id}] ${valenciaTeam.nombre}`);
  ...
  console.log(`\nJugadores seleccionados con peso en la última fecha (${latestDate}): ${playersWithWeightOnDate.length}`);
