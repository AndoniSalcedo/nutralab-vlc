...
export function parseExcelDate(value, { preferDayFirst = true } = {}) {
  ...
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    return asIsoDate(value.getFullYear(), value.getMonth() + 1, value.getDate());
  }

  if (typeof value === 'number') {
    const parsed = XLSX.SSF.parse_date_code(value);
    return parsed ? asIsoDate(parsed.y, parsed.m, parsed.d) : null;
  }

  const raw = cleanText(value);
  if (!raw || /^0{1,2}[-/]0{1,2}[-/]0{1,4}$/.test(raw)) return null;
  ...
  if (match) {
    ...
    if (a > 12) return asIsoDate(year, b, a);
    if (b > 12) return asIsoDate(year, a, b);
    return preferDayFirst ? asIsoDate(year, b, a) : asIsoDate(year, a, b);
  }

  const parsed = new Date(raw);
  if (Number.isNaN(parsed.getTime())) return null;
  return asIsoDate(parsed.getFullYear(), parsed.getMonth() + 1, parsed.getDate());
}
...
function toNumber(value, { zeroAsNull = true } = {}) {
  if (value === null || value === undefined || value === '') return null;
  if (typeof value === 'number') {
...
export function parsePlayerExcel(buffer) {
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    for (let rowIndex = headerRowIndex + 1; rowIndex < rows.length; rowIndex += 1) {
      ...
      const fecha = sheetDate || originalDate;
      if (!isValidMeasurementDate(fecha)) continue;
      ...
      const dateCorrected = Boolean(sheetDate && originalDate && sheetDate !== originalDate);

      records.push({
        key: `${normalizedName}|${fecha}`,
...
function groupRecordsByPlayer(records) {
  ...
  return Array.from(byPlayer.values()).map((group) => ({
    ...group,
    mediciones: group.mediciones.sort((a, b) => a.fecha.localeCompare(b.fecha)),
    warnings: Array.from(new Set(group.warnings)),
  }));
}

function playerFullName(player) {
  return cleanText(`${player?.nombre || ''} ${player?.apellidos || ''}`);
}

function partialCandidates(group, players) {
  const groupTokens = group.key.split(' ').filter((token) => token.length > 2);
...
export function buildImportPlan(parsed, players = []) {
  return parsed.groups.map((group) => {
    ...
    if (exactMatches.length === 1) {
      ...
      candidates = partialCandidates(group, players);
    ...
    const ultimaFecha = group.mediciones.at(-1)?.fecha || null;
