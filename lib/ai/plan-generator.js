...
export function calculateMealBudgets(day) {
  ...
  // Porcentajes de reparto de macronutrientes según la estructura exacta de tomas
  let pPcts = {};
  let hcPcts = {};
  let gPcts = {};
  ...
  if (hasPost) {
    if (hasDesayuno && hasMerienda && hasComida && hasCena) {
      // 5 tomas: Desayuno 10%, Comida 35%, Merienda 10%, Cena 35%, Post 10%
      pPcts = { Desayuno: 0.10, Comida: 0.35, Merienda: 0.10, Cena: 0.35, Post: 0.10 };
      hcPcts = { Desayuno: 0.20, Comida: 0.35, Merienda: 0.15, Cena: 0.30, Post: 0.00 };
      gPcts = { Desayuno: 0.18, Comida: 0.35, Merienda: 0.12, Cena: 0.35, Post: 0.00 };
    } else if (hasDesayuno && !hasMerienda && hasComida && hasCena) {
      // 4 tomas: Desayuno 15%, Comida 40%, Cena 35%, Post 10%
      pPcts = { Desayuno: 0.15, Comida: 0.40, Cena: 0.35, Post: 0.10 };
      hcPcts = { Desayuno: 0.25, Comida: 0.40, Cena: 0.35, Post: 0.00 };
      gPcts = { Desayuno: 0.20, Comida: 0.40, Cena: 0.40, Post: 0.00 };
    } else if (!hasDesayuno && !hasMerienda && hasComida && hasCena) {
      // 3 tomas (ej: Crettaz): Comida 45%, Cena 45%, Post 10%
      pPcts = { Comida: 0.45, Cena: 0.45, Post: 0.10 };
      hcPcts = { Comida: 0.50, Cena: 0.50, Post: 0.00 };
      gPcts = { Comida: 0.50, Cena: 0.50, Post: 0.00 };
    } else {
      // Fallback proporcional con Post al 10%
      const others = mealNames.filter(n => !n.toLowerCase().includes('post'));
      const share = 0.90 / (others.length || 1);
      pPcts = { Post: 0.10 };
      hcPcts = { Post: 0.00 };
      gPcts = { Post: 0.00 };
      others.forEach(n => {
        pPcts[n] = share;
        hcPcts[n] = 1.0 / others.length;
        gPcts[n] = 1.0 / others.length;
      });
    }
  } else {
    ...
    if (hasDesayuno && hasMerienda && hasComida && hasCena) {
      ...
      pPcts = { Desayuno: 0.15, Comida: 0.375, Merienda: 0.10, Cena: 0.375 };
      hcPcts = { Desayuno: 0.22, Comida: 0.36, Merienda: 0.16, Cena: 0.26 };
      gPcts = { Desayuno: 0.20, Comida: 0.35, Merienda: 0.15, Cena: 0.30 };
    } else if (hasDesayuno && !hasMerienda && hasComida && hasCena) {
      // 3 tomas sin post: Desayuno 20%, Comida 42%, Cena 38%
      pPcts = { Desayuno: 0.20, Comida: 0.42, Cena: 0.38 };
      hcPcts = { Desayuno: 0.25, Comida: 0.40, Cena: 0.35 };
      gPcts = { Desayuno: 0.22, Comida: 0.40, Cena: 0.38 };
    } else if (!hasDesayuno && !hasMerienda && hasComida && hasCena) {
      // 2 tomas sin post: Comida 50%, Cena 50%
      pPcts = { Comida: 0.50, Cena: 0.50 };
      hcPcts = { Comida: 0.50, Cena: 0.50 };
      gPcts = { Comida: 0.50, Cena: 0.50 };
    } else {
  ...
  for (const ing of ingestas) {
    ...
    const pFrac = pPcts[key] ?? (1 / ingestas.length);
    const hcFrac = hcPcts[key] ?? (1 / ingestas.length);
    const gFrac = gPcts[key] ?? (1 / ingestas.length);

    const mP = Math.round(proteina * pFrac);
  ...
  for (const opt of options) {
    ...
    if (hasRestrictions) {
      ...
      if (!isExcluded && isFishRestricted) {
        ...
        if (category === 'pescados_y_mariscos' || fishKeywords.some(kw => optLow.includes(kw) || resolvedLow.includes(kw))) {
          isExcluded = true;
        }
      }
