...
export function filterDishOptionsByRestrictions(dishText, jugador) {
  ...
  const isGlutenRestricted = rawRestr.includes('gluten') || rawRestr.includes('celiac');
  const isLactoseRestricted = rawRestr.includes('lactosa') || rawRestr.includes('leche');
...
export async function generarDatosPlan({ jugador, nombre, contexto, contextoAdicional, calendario, menu, teamConfig, recomendacionesIngestas, preMatchConfig }) {
  ...
  const isLactoseIntolerant = activeTagsSet.has('sin_lactosa');
  const isGlutenIntolerant = activeTagsSet.has('sin_gluten');
  ...
  const calOptions = { isLactoseIntolerant, isGlutenIntolerant, isFishIntolerant, isPorkIntolerant, clinicalCatalog };
