...
function buildCompleteResult(rawText, mealName, jugador, isPreMatch) {
  const branches = getCompleteMealBranches(mealName, null, jugador, isPreMatch);
  return {
    raw: rawText,
    isComplete: true,
    proteina: [],
...
function buildBranches(items, mealName, jugador, isPreMatch) {
  const branches = [];
  ...
  return branches.length > 0 ? branches : getCompleteMealBranches(mealName, null, jugador, isPreMatch);
}

function buildStructuredResult(rawText, item, mealName, jugador, isPreMatch) {
  ...
  return {
    ...
    branches: alternativas.length > 0
      ? []
      : buildBranches(allItems, mealName, jugador, isPreMatch),
    unrecognized: [],
    isValid: true,
    label,
  };
...
export async function parseMealTreeWithAI({
  ...
  Object.entries(mealsToProcess).forEach(([currentMealName, rawText]) => {
    if (isCompleteMealText(rawText)) {
      results[currentMealName] = buildCompleteResult(rawText, currentMealName, jugador, isPreMatch);
    } else {
