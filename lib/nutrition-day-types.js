...
export function getUserMeals(jugador) {
  ...
  if (jugador.num_comidas) {
    if (!isNaN(Number(jugador.num_comidas))) {
      ...
      const order = [...STANDARD_MEALS, 'Pre-entreno', 'Post-entreno'];
      meals.sort((a, b) => {
  ...
  if (jugador.preentreno) {
    const hasPre = meals.some(
      (m) => m.toLowerCase() === 'pre-entreno' || m.toLowerCase() === 'pre entreno' || m.toLowerCase() === 'pre'
    );
    if (!hasPre) {
      meals.push('Pre-entreno');
    }
  }
...
export function getUserMealsForDay(jugador, tipoDia, teamConfig) {
  ...
  const hasPreentrenoEnabled = jugador.preentreno;
  if (!hasPreentrenoEnabled) {
    return baseMeals.filter((meal) => {
      const isPre = meal.toLowerCase() === 'pre-entreno' || meal.toLowerCase() === 'pre entreno' || meal.toLowerCase() === 'pre';
      return !isPre;
    });
  }
  ...
  let tienePreentreno = false;
  if (dayTypeConfig) {
    if (dayTypeConfig.tienePreentreno !== undefined) {
      tienePreentreno = dayTypeConfig.tienePreentreno;
    } else {
      tienePreentreno = ['doble', 'entreno', 'partido'].includes(dayTypeConfig.key);
    }
  }
