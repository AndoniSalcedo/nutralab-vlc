  ...
  if (tipoDia === 'partido' && preMatchConfig?.enabled) {
    ...
    if (matchConfig && Array.isArray(matchConfig.ingestas) && matchConfig.ingestas.length > 0) {
      ...
      if (hasPost && !isAlreadyPost) {
        ...
        return matchMeals.filter(m => !(m.toLowerCase() === 'post-entreno' || m.toLowerCase() === 'post entreno' || m.toLowerCase() === 'post' || m.toLowerCase() === 'post-partido' || m.toLowerCase() === 'post partido'));
      }
      return matchMeals;
    }
  }
