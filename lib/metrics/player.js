import { metricValue } from './measurements.js';
...
export function extractMasaMagra(record) {
  ...
  const calcFromFat = (Number.isFinite(peso) && peso > 0 && Number.isFinite(grasa) && grasa > 0 && grasa < 100)
    ? Math.round(peso * (1 - grasa / 100) * 100) / 100
    : null;

  for (const k of ['peso_magro', 'masa_magra_kg']) {
    if (hasMetricValue(record[k])) {
      const val = Number(record[k]);
      if (Number.isFinite(val) && val > 0) {
        // Sanity check: lean mass in living humans is > 65% of weight. If someone stored muscle mass (~45%), use calcFromFat
        if (Number.isFinite(peso) && peso > 0 && val < peso * 0.60 && calcFromFat !== null) {
          return calcFromFat;
        }
  ...
  if (record.metricas_excel && typeof record.metricas_excel === 'object') {
    for (const k of ['Peso Magro', 'Masa Magra', 'Masa magra (kg)', 'Masa Magra (kg)', 'PESO MAGRO', 'PESO MAGRO (Kg)']) {
...
export function getLatestMasaMagra(evoluciones = [], fallback = null) {
  ...
  if (fallback !== null && fallback !== undefined && fallback !== '') {
    const fbVal = extractMasaMagra(typeof fallback === 'object' ? fallback : { peso_magro: fallback, masa_magra_kg: fallback });
...
export function calculateSemaforo(pesajes = [], currentWeightOverride = null, options = {}) {
  ...
  // Cross-check con la última evolución: la masa magra real debe ser peso * (1 - grasa / 100)
  const sortedEvos = [...evolucionesList].filter((e) => e?.fecha).sort((a, b) => compareDates(a.fecha, b.fecha));
  const latestEvo = sortedEvos.length > 0 ? sortedEvos[sortedEvos.length - 1] : null;
  if (latestEvo) {
    const evoWeight = Number(latestEvo.peso_kg);
    const evoFat = Number(latestEvo.porcentaje_grasa ?? latestEvo.porcentaje_grasa_faulkner);
    if (Number.isFinite(evoWeight) && evoWeight > 0 && Number.isFinite(evoFat) && evoFat > 0 && evoFat < 100) {
      const calcLean = Math.round(evoWeight * (1 - evoFat / 100) * 100) / 100;
      if (!masaMagra || masaMagra < evoWeight * 0.60) {
        masaMagra = calcLean;
      }
    }
  }
  ...
  targetFatPct = Math.round(targetFatPct * 100) / 100;
...
export function withLatestMeasurement(jugador, evoluciones = [], pesajes = []) {
  ...
  if (latestEvolucion?.fecha && latestPesaje?.fecha) {
    latestDate = compareDates(latestEvolucion.fecha, latestPesaje.fecha) > 0 ? latestEvolucion.fecha : latestPesaje.fecha;
  } else {
  ...
  const latestMasaMagra = getLatestMasaMagra(evoluciones, jugador.peso_magro || jugador.masa_magra_kg);
  const rawTargetFat = jugador.porcentaje_grasa_objetivo !== undefined && jugador.porcentaje_grasa_objetivo !== null && jugador.porcentaje_grasa_objetivo !== ''
    ? Number(jugador.porcentaje_grasa_objetivo)
    : 10;
  const targetFatPct = Number.isFinite(rawTargetFat) && rawTargetFat > 0 && rawTargetFat < 100
    ? Math.round(rawTargetFat * 100) / 100
    : 10;
  ...
  return {
    ...
    peso_magro: latestMasaMagra,
    masa_magra_kg: latestMasaMagra,
    porcentaje_grasa_objetivo: targetFatPct,
    porcentaje_musculo: latestMetricValue(evoluciones, 'porcentaje_musculo', jugador.porcentaje_musculo),
