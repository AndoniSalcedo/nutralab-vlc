...
export function resolveDishIngredientsForPlayer(decomposedDish, clinicalCatalog) {
  ...
  if (decomposedDish.grasa) {
    const resolvedGrasa = resolveNodeForPlayer(decomposedDish.grasa, clinicalCatalog) || 'AOVE';
...
export function resolveMealTreeForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false) {
  ...
  const carbRotation = isPreMatch
    ? ['Arroz blanco', pastaApta, 'Patata', 'Tortitas de arroz']
    : ['Arroz blanco', pastaApta, 'Patata', 'Boniato', 'Quinoa', 'Arroz basmati'];
  ...
  if (!hasSinPescado && !isPreMatch) proteinRotation.push('Salmón');
  proteinRotation.push('Ternera magra');
  if (!hasSinPescado) proteinRotation.push('Merluza');
  proteinRotation.push('Pechuga de pavo');
  proteinRotation.push('Huevo entero');
  if (!hasSinCerdo && !isPreMatch) proteinRotation.push('Solomillo de cerdo');

  // 3. Opciones de Verduras disponibles para rotar
  const veggieRotation = isPreMatch
    ? ['Calabacín', 'Zanahoria']
    : ['Calabacín', 'Brócoli', 'Judías verdes', 'Espinaca', 'Tomate'];
