...
export function getClinicalCatalogForPlayer(playerOrTags) {
  ...
  const filteredFoods = FOODS_CRUDO.filter((f) => {
    ...
    const isPlantProtein = foodName.includes('tofu') || foodName.includes('seit') || foodName.includes('seitan') || foodName.includes('soja texturizada');
    ...
    if (isVegetarian) {
      if (f.category === 'carnes_y_aves' || f.category === 'pescados_y_mariscos' || f.category === 'conservas') return false;
    }
