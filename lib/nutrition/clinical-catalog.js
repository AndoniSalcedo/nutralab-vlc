...
export function getClinicalCatalogForPlayer(playerOrTags) {
  ...
  const filteredFoods = FOODS_CRUDO.filter((f) => {
    ...
    if (hasLactoseRestriction) {
      ...
      // Si el jugador NO tiene restricción de lactosa ni de APLV, purgar productos especiales sin lactosa
    ...
    if (hasCowProteinRestriction) {
      if (foodTags.includes('proteina_vaca') || foodTags.includes('lactosa') || foodTags.includes('sin_lactosa_especial')) {
        // Permitir batido vegetal seguro si estuviera marcado con sin_lactosa_especial
        if (f.name.toLowerCase().includes('vegetal')) {
          // Apto
        } else {
          return false;
        }
