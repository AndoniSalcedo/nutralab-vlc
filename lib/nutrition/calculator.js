...
export function findFoodBySimilarity(query, options = {}) {
  ...
  for (const food of FOODS_CRUDO) {
    ...
    // Priorizaciones clínicas y culinarias universales:
    // 1. Pasta: si el jugador no tolera gluten -> Pasta sin gluten; de lo contrario -> Pasta de trigo
    if (queryTokens.includes('pasta') || normQuery === 'pasta') {
      if (options.isGlutenIntolerant && normFood === 'pasta sin gluten') totalScore += 0.5;
      else if (!options.isGlutenIntolerant && normFood === 'pasta de trigo') totalScore += 0.5;
    }

    // 2. Pollo: "pollo asado", "pollo al horno" -> Pechuga de pollo
    if (queryTokens.includes('pollo') && normFood === 'pechuga de pollo') {
      totalScore += 0.35;
    }

    // 3. Pavo: "pavo pechuga" -> Pechuga de pavo
    ...
    if ((normQuery.includes('batido') || normQuery.includes('recovery')) && normFood.startsWith('batido de proteina')) {
      if (options.isLactoseIntolerant && normFood.includes('sin lactosa')) totalScore += 0.4;
      else if (!options.isLactoseIntolerant && !normFood.includes('sin lactosa')) totalScore += 0.4;
    }
...
export function findFoodInCatalog(query, options = {}) {
  ...
  const exact = FOODS_BY_NORMALIZED_NAME.get(norm);
  if (exact) return exact;

  const cacheKey = `${norm}${options.isGlutenIntolerant ? '_gf' : ''}${options.isLactoseIntolerant ? '_lf' : ''}`;
...
export async function calibrateMeal(mealDetailStr, targetBudget, options = {}) {
  ...
  if (options.isGlutenIntolerant) {
    parsedItems.forEach(it => {
      const itLow = it.name.toLowerCase();
      if (itLow.includes('pan blanco') || itLow.includes('pan de barra') || itLow.includes('pan integral')) {
        console.warn(`[SEGURIDAD CLÍNICA] Sustituyendo "${it.name}" por Pan sin gluten debido a intolerancia al gluten.`);
        it.name = 'Pan sin gluten';
        it.food = findFoodInCatalog('Pan sin gluten') || CATEGORY_FALLBACKS.cereales_y_tuberculos;
      } else if ((itLow.includes('pasta') && !itLow.includes('sin gluten')) || itLow.includes('fideua') || itLow.includes('fideuà') || itLow.includes('cuscus') || itLow.includes('cuscús') || itLow.includes('cous')) {
        console.warn(`[SEGURIDAD CLÍNICA] Sustituyendo "${it.name}" por Pasta sin gluten debido a intolerancia al gluten.`);
        it.name = 'Pasta sin gluten';
        it.food = findFoodInCatalog('Pasta sin gluten', options) || CATEGORY_FALLBACKS.cereales_y_tuberculos;
      }
    });
  }

  if (options.isLactoseIntolerant) {
    parsedItems.forEach(it => {
      const itLow = it.name.toLowerCase();
      if ((itLow.includes('leche') || itLow.includes('yogur') || itLow.includes('queso')) && !itLow.includes('sin lactosa')) {
        console.warn(`[SEGURIDAD CLÍNICA] Adaptando lácteo "${it.name}" a versión sin lactosa.`);
        if (itLow.includes('leche')) it.name = 'Leche entera sin lactosa';
        else if (itLow.includes('queso fresco')) it.name = 'Queso fresco sin lactosa';
        else if (itLow.includes('yogur')) it.name = 'Yogur natural sin lactosa';
        it.food = findFoodInCatalog(it.name) || it.food;
      }
    });
  }
