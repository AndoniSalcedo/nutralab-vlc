...
export function isEggItem(it) {
  ...
  return (
    nameLow === 'huevo entero' ||
    nameLow === 'huevo' ||
    nameLow === 'huevos' ||
    nameLow.includes('huevo entero') ||
    (it.food?.tags?.includes('huevo') && !nameLow.includes('clara')) ||
...
export function formatMealItemDisplayName(it) {
  ...
  // Manejo de presentaciones especiales en unidades físicas
  ...
  let canonicalName = (it.food?.name || it.name || '').trim();
