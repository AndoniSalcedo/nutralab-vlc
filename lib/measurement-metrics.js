export const TREND_MEASUREMENT_METRICS = [
  ...
  { key: 'peso_muscular', label: '% Músculo (Lee)', unit: '%', color: '#16a34a', goodDown: false, excelKeys: ['%Peso Muscular Lee&cols', '% Peso Muscular Lee&cols'] },
...
export const MEASUREMENT_DETAIL_SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'peso_graso', label: 'Masa grasa', unit: 'kg' },
      { key: 'peso_muscular', label: 'Masa muscular', unit: 'kg' },
      { key: 'peso_magro', label: 'Masa magra', unit: 'kg' },
...
export function metricValue(record, metric) {
  ...
  if (metric.excelKeys) {
    const rawExcel = record.metricas_excel;
    if (rawExcel && typeof rawExcel === 'object') {
      for (const k of metric.excelKeys) {
        if (hasMetricValue(rawExcel[k])) return rawExcel[k];
      }
    }
  }

  // Custom fallback calculation for percentage of muscle mass
  if (metric.key === 'peso_muscular' && metric.unit === '%') {
    if (hasMetricValue(record.peso_muscular) && hasMetricValue(record.peso_kg)) {
