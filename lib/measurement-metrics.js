export const TREND_MEASUREMENT_METRICS = [
  ...
  { key: 'peso_muscular', label: '% Peso Muscular Lee&cols', unit: '%', color: '#16a34a', goodDown: false },
...
export function metricValue(record, metric) {
  ...
  // Custom rule for percentage of muscle mass
  if (metric.key === 'peso_muscular' && metric.unit === '%') {
    const rawExcel = record.metricas_excel;
    if (rawExcel && typeof rawExcel === 'object') {
      const excelKeys = ['%Peso Muscular Lee&cols', '% Peso Muscular Lee&cols', 'PESO MÚSCULO (%) (Lee&cols)'];
