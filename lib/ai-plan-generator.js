...
export function buildWeeklyPrompt({ jugador, baseData, menu, preMatchConfig, contextoAdicional, recomendacionesIngestas }) {
  ...
  const payload = {
    ...
    reglas_calidad_nutricional_y_variedad: [
      ...
      '4. SEGURIDAD CLÍNICA TOTAL Y ALIMENTOS PROHIBIDOS: Respeta rigurosamente alergias, intolerancias, aversiones y contexto médico del jugador. Jamás incluyas un alimento prohibido o conflictivo. PROHIBICIÓN ABSOLUTA: Si el jugador tiene aversión o intolerancia a un alimento (como cerdo o lentejas para Eray, o gluten y lactosa para Gonzalo), ese alimento NUNCA debe aparecer en el menú bajo ningún concepto. PROHIBIDO escribir "X sustituido por Y" (ej: JAMÁS escribir "lomo de cerdo magro sustituido por pechuga de pavo", escribe DIRECTAMENTE "Pechuga de pavo 160g"). PROHIBIDO escribir alimentos con 0g (ej: JAMÁS escribir "Pasta de trigo 0g, quinoa 210g", escribe DIRECTAMENTE "Quinoa 210g").',
      '5. DENSIDAD Y CUADRE MATEMÁTICO REAL DE HIDRATOS (PROHIBIDO QUEDARSE CORTO EN HC): Calcula con rigor los gramos de alimentos usando la tabla de composición nutricional para cumplir exactamente las macros fijadas en cada ingesta (tolerancia máxima ±5%). ¡ATENCIÓN CRÍTICA CON LOS TUBÉRCULOS!: La patata y el boniato tienen solo un ~18-20% de carbohidratos (el 80% es agua). ¡100g de patata aportan solo 18g de HC! Si una comida requiere 90-110g de HC y usas patata, debes poner ~500-550g de patata cruda (o combinar 250g de patata con pan, fruta o arroz). En el desayuno: si requiere 70-80g de HC, 60g de pan no bastan (solo aportan 27g HC); debes usar ~120g de pan integral + 150-200g de fruta (o copos de avena con fruta). En la merienda: si requiere 50-60g de HC, 40g de tortas no bastan (solo aportan 31g HC); debes usar ~70-80g de tortas + fruta o miel. DISTRIBUYE los hidratos de forma equilibrada entre todas las ingestas del día. Si la carne/pescado no alcanza la proteína marcada, añade complementos proteicos limpios (claras de huevo, yogur proteico/sin lactosa, queso fresco batido, huevo cocido, lata de atún al natural). Queda TERMINANTEMENTE PROHIBIDO dejar déficit de hidratos de carbono en el día.',
      '6. SOLO INGREDIENTES Y GRAMAJES (SIN MÉTODOS DE COCINADO NI "EN CRUDO"): Nombra únicamente los alimentos y sus gramos exactos. Elige alimentos individuales reales (ej: "sepia 200g", "merluza 200g", "dorada 200g", "manzana 150g", "nueces 20g"); PROHIBIDO nombrar mezclas híbridas en un solo ítem (no escribir "sepia y calamar", "merluza bacalao", "manzana y pera" ni "pollo y pavo"). PROHIBIDO añadir métodos de cocinado ("a la plancha", "al horno", "hervido") y PROHIBIDO escribir "(en crudo)". Ejemplo: "Arroz blanco 220g, solomillo de ternera 200g, calabacín y zanahoria 150g, AOVE 15g, plátano 150g".',
    ...
    for (const dayKey of daysOfWeek) {
      ...
      if (aiDay?.ingestas && Array.isArray(aiDay.ingestas) && aiDay.ingestas.length > 0) {
        finalDias[dayKey] = {
          ...
          ingestas: aiDay.ingestas.map((ing) => ({
            ...ing,
            detalle: sanitizeMealDetail(ing.detalle),
          })),
        };
