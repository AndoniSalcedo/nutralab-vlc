...
function preMatchToPrompt(preMatchConfig, planOrBaseData, jugador = null) {
  ...
  for (const matchDayKey of matchDayKeys) {
    ...
    const recsList = Object.entries(recs).filter((entry) => entry[1] && entry[1].trim() !== '');
    const diaAnterior = (playerConfig.dia_anterior || '').trim();
    const textoGlobal = matchDayConfig.texto ? matchDayConfig.texto.trim() : '';

    if (recsList.length === 0 && !diaAnterior && !textoGlobal && !jugador?.config_prepartido?.[horario]?.ingestas) {
      continue;
    }
    ...
    const scheduleName = horario === 'manana' ? 'MAÑANA' : horario === 'noche' ? 'NOCHE' : 'TARDE';

    let scheduleDescription = '';
    if (horario === 'manana') {
      scheduleDescription = `* En ${prevDayLabel} (día anterior): enfocar la carga en la Comida, Merienda y Cena.\n* En ${matchDayLabel} (día de partido): comida pre-partido en el Desayuno o Almuerzo ligero.`;
    } else if (horario === 'noche') {
      scheduleDescription = `* En ${prevDayLabel} (día anterior): carga completa terminando con una buena Cena.\n* En ${matchDayLabel} (día de partido): Desayuno, Comida normal y Merienda o cena ligera pre-partido unas horas antes.`;
    } else {
      scheduleDescription = `* En ${prevDayLabel} (día anterior): afecta a la Cena (y Merienda si existe).\n* En ${matchDayLabel} (día de partido): afecta al Desayuno, Almuerzo y Comida pre-partido.`;
    }

    let pautaDetails = '';
    if (recsList.length > 0) {
      pautaDetails += `\nRECOMENDACIONES INDIVIDUALES PARA EL DÍA DE PARTIDO (${matchDayLabel}):\n` + 
        recsList.map(([meal, rec]) => `  - ${meal}: ${rec}`).join('\n');
    }
    if (diaAnterior) {
      pautaDetails += `\nPAUTA INDIVIDUAL PARA EL DÍA ANTERIOR (${prevDayLabel} - 24H PREVIAS):\n  "${diaAnterior}"`;
    }
    if (!recsList.length && !diaAnterior && textoGlobal) {
      pautaDetails += `\nINDICACIONES PRE-PARTIDO DEL EQUIPO:\n  "${textoGlobal}"`;
    }

    parts.push(
      `--- PAUTA PRE-PARTIDO 24H PARA EL PARTIDO DEL ${matchDayLabel.toUpperCase()} (HORARIO: ${scheduleName}) ---\n` +
      `Ventana de 24h previas y timing:\n${scheduleDescription}` + pautaDetails
    );
  }
  ...
  return [
    ...
    `1. PRIORIDAD DE RUTINA INDIVIDUAL: Si se incluyen recomendaciones individuales para el día de partido o para el día anterior (24h previas), tienen PRIORIDAD ABSOLUTA sobre el menú de ciudad deportiva o rutinas habituales.`,
    `2. ADAPTACIÓN A LAS INGESTAS DEL JUGADOR: La estructura de ingestas del día del partido en el JSON de entrada ya refleja con precisión el número y nombre de las comidas que el jugador realiza específicamente para un partido en este horario. Respeta estrictamente esas ingestas y calcula gramos en crudo de acuerdo a sus macros y calorías.`,
    `3. TIMING SEGÚN HORARIO: Ajustar los carbohidratos y la digestión en función del horario de partido indicado (Mañana, Tarde o Noche).`,
  ].join('\n');
}
...
function buildPrompt({ jugador, contextoAdicional, menu, baseData, retry = false, recomendacionesIngestas, preMatchConfig }) {
  ...
  const preMatchPrompt = preMatchToPrompt(preMatchConfig, baseData, jugador);
  ...
  return [
    ...
    '',
    'Reglas:',
    '- Cada detalle debe ser una frase corta y concisa.',
    '- ES ESTRICTAMENTE OBLIGATORIO indicar siempre el peso en GRAMOS (g) para TODOS y cada uno de los alimentos e ingredientes que aparezcan en la ingesta (incluyendo frutas, verduras, lácteos, cereales, aceite, pan, etc.). No uses unidades informales como "un bol", "una pieza" o "una rebanada" si puedes expresar su equivalente en gramos. Por ejemplo, escribe "Manzana (150 g)" en lugar de "1 Manzana", y "Aceite de oliva virgen extra (4 g)" en lugar de "un chorrito".',
    '- REGLA CRÍTICA DE CUADRE MATEMÁTICO REAL DE CALORÍAS Y MACROS (MACRO & CALORIE BUDGETING):',
    '  * Para cada día se te proporcionan las "kcal", "proteina" (P), "hidratos" (HC) y "grasa" (G) totales ya calculadas. Los alimentos y gramajes que redactes en el texto DEBEN SUMAR REALMENTE esas calorías y macronutrientes asignados al día (margen de tolerancia máximo ±5%).',
    '  * NUNCA pongas porciones visuales pequeñas por defecto (ej. 80-90 g de arroz o 10 g de aceite cuando el jugador necesita 2.500-3.500 kcal y solo tiene 2 comidas al día).',
    '  * PASO 1 - Reparto del presupuesto por ingesta:',
    '    - Si existe ingesta "Post-entreno", su contenido es exclusivamente proteína aislada en polvo (~30-35 g en 250-300 ml de agua = ~115-130 kcal, ~25-30 g P). Descuenta esta proteína antes de presupuestar las comidas principales.',
    '    - Si existe ingesta pre-entreno / merienda previa a partido, presupuesta alimentos energéticos de fácil digestión (~150-250 kcal, ~35-60 g HC como tortas de arroz, miel, plátano).',
    '    - Reparte el remanente de kcal, P, HC y G entre las comidas principales según su peso lógico (ej. en 2 comidas principales: ~50% Comida y ~50% Cena; en 3 comidas: ~25% Desayuno, ~40% Comida, ~35% Cena; en 4 comidas: ~20% Desayuno, ~35% Comida, ~15% Merienda, ~30% Cena).',
    '  * PASO 2 - Cálculo matemático de los gramos de cada ingrediente del plato (siempre en peso EN CRUDO):',
    '    - Fuentes de carbohidratos en crudo: Arroz blanco, pasta, quinoa, cuscús, avena en crudo aportan ~75 g HC y ~360 kcal por 100 g. Patata y boniato aportan ~20 g HC y ~85 kcal por 100 g. Legumbres secas aportan ~60 g HC y 22 g P por 100 g.',
    '    - Fuentes de proteína en crudo: Pechuga de pollo, pavo, ternera magra, solomillo, atún fresco, merluza, salmón en crudo aportan ~20-22 g P y ~110-160 kcal por 100 g. Huevos aportan ~6.5 g P y 5 g G por unidad (~60 g).',
    '    - Fuentes de grasas: Aceite de oliva virgen extra (AOVE) es 100% grasa (9 kcal/g, 10 g = 90 kcal, 20 g = 180 kcal, 30 g = 270 kcal). Aguacate aporta ~15 g grasa por 100 g. Frutos secos aportan ~50 g grasa y ~18 g P por 100 g.',
    '    - Fuentes complementarias de hidratos densos: Fruta fresca (plátano 150 g = ~35 g HC, manzana/pera 150 g = ~20 g HC), tortas de arroz (50 g = ~40 g HC), miel (20 g = ~16 g HC), pan (60 g = ~30 g HC). Úsalas cuando el volumen de cereal o tubérculo sea elevado para que la comida sea agradable y digestiva.',
    '  * PASO 3 - Verificación de la suma:',
    '    - Asegúrate de que la suma de los alimentos de cada ingesta cubra con rigor el presupuesto de calorías y macronutrientes fijado para esa comida y para el día.',
    ...
    '- Evita strictly alergias, intolerancias y aversiones. No añadas aclaraciones ni coletillas redundantes como "sin gluten" a alimentos que son naturalmente libres de gluten por su propia naturaleza (como arroz, patata, carne, pescado, fruta o verdura). Reservar "sin gluten" únicamente para productos procesados o sustitutivos de alimentos que originalmente sí contienen gluten (ej. "pan sin gluten", "pasta sin gluten").',
    ...
    '- Todas las cantidades en gramos para alimentos que se cocinan (arroz, pasta, carne, pescado, legumbres, etc.) deben indicarse SIEMPRE en peso EN CRUDO (nunca cocinado).',
...
function buildReviewPrompt({ jugador, contextoAdicional, menu, planData, recomendacionesIngestas, preMatchConfig }) {
  const recsPrompt = recommendationsToPrompt(recomendacionesIngestas);
  const preMatchPrompt = preMatchToPrompt(preMatchConfig, planData, jugador);

  return [
    'Eres Carlos Ferrando, nutricionista del Valencia CF.',
    'Revisa esta ficha nutricional ya generada y corrige SOLO las ingestas que entren en conflicto con alergias, intolerancias, aversiones, contexto clínico, pauta de 24h pre-partido, recomendaciones del nutricionista o déficit/descuadre calórico y de macronutrientes.',
    ...
    '- AUDITORÍA MATEMÁTICA Y CUADRE CALÓRICO OBLIGATORIO: Audita la suma real de calorías y macronutrientes de los alimentos generados en cada ingesta frente a las "kcal", "proteina", "hidratos" y "grasa" asignadas al día. Si la suma de los alimentos en el texto está por debajo o por encima del objetivo del día (por ejemplo, si los gramos de carbohidratos en crudo, carne/pescado o AOVE son demasiado bajos y no cubren las calorías marcadas), DEBES corregir y aumentar/ajustar los gramos en crudo y/o añadir complementos saludables (frutas, tortas de arroz, miel, AOVE) hasta que la suma matemática del texto cuadre con las calorías y macros de la cabecera (margen ±5%).',
    ...
    '- No menciones el alimento prohibido en el detalle final; escribe solo la alternativa segura (evitando coletillas redundantes como "sin gluten" en alimentos naturalmente libres de gluten por su propia naturaleza, como el arroz o la patata).',
    ...
    '- ES ESTRICTAMENTE OBLIGATORIO que todos los alimentos e ingredientes de las ingestas tengan su peso explícito en GRAMOS (g). Si algún alimento en la ficha generada carece de gramos o usa unidades informales (ej: "1 manzana", "un bol"), DEBES corregirlo para incluir sus gramos exactos (ej: "Manzana (150 g)").',
    '- Todas las cantidades en gramos para alimentos que se cocinan deben indicarse SIEMPRE en peso EN CRUDO.',
    '- Salvo para corregir faltas de gramos, descuadres calóricos o resolver conflictos de salud/pre-partido, mantén el resto de la estructura y contenido de las ingestas tal como están.',
    '',
    'Jugador:',
    playerContextToPrompt(jugador),
    '',
