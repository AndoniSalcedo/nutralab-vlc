...
export async function reviewAndRepairPlanData({ jugador, contexto, contextoAdicional, menu, planData, teamConfig }) {
  try {
...
export async function generarDatosPlan({ jugador, nombre, contexto, contextoAdicional, calendario, menu, teamConfig }) {
  const supabase = getSupabaseAdmin();
  const resolvedMenu = menu !== undefined ? menu : await latestMenu(supabase, jugador?.equipo_id);
  const baseData = buildBasePlanData({ jugador, nombre, contexto, contextoAdicional, menu: resolvedMenu, calendario, teamConfig });
