...
function menuToPrompt(menu, dayKey = null) {
...
function playerContextToPrompt(jugador) {
  return [
    ...
    `Número de comidas configuradas: ${jugador.num_comidas || 'Comida, Cena'}${jugador.postentreno ? ' (+ Post-entreno en días de entreno/partido)' : ''}`,
...
export function calculateMealBudgets(day) {
  ...
  let remKcal = kcal || 2400;
  let remP = proteina || 150;
  let remHC = hidratos || 300;
  let remG = grasa || 70;
...
export function buildMealAssemblySpec({ jugador, dayKey, dayData, menu, preMatchConfig, recsForDay, mealBudgets }) {
...
export function buildDayPrompt({ jugador, dayKey, dayData, menu, preMatchConfig, contextoAdicional, recsForDay }) {
  ...
  const payload = {
    ...
    jugador: {
      ...
      posicion: jugador.posicion || 'Jugador',
      peso_kg: jugador.peso_kg,
      objetivo: getObjectiveLabel(jugador.objetivo) || jugador.objetivo,
      contexto_clinico: jugador.contexto_clinico || 'Sin particularidades',
      alergias_intolerancias: [jugador.alergias ? `Alergias: ${jugador.alergias}` : null, jugador.intolerancias ? `Intolerancias: ${jugador.intolerancias}` : null, jugador.aversiones ? `Aversiones: ${jugador.aversiones}` : null].filter(Boolean).join(' | ') || 'Ninguna',
...
export async function generateSingleDay({ jugador, dayKey, dayData, menu, preMatchConfig, contextoAdicional, recsForDay }) {
  ...
  const rawIngestas = json.ingestas || dayData.ingestas || [];
...
function buildWeeklyNotesPrompt({ jugador, planData, preMatchConfig, contextoAdicional }) {
  return [
