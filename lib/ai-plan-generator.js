...
function buildPrompt({ jugador, contextoAdicional, menu, baseData, retry = false, recomendacionesIngestas }) {
  ...
  return [
    ...
    '- Comida y cena deben usar como base el menú de ciudad deportiva cuando exista, sirviendo como referencia.',
...
function buildReviewPrompt({ jugador, contextoAdicional, menu, planData, recomendacionesIngestas }) {
  const recsPrompt = recommendationsToPrompt(recomendacionesIngestas);

  return [
    'Eres Carlos Ferrando, nutricionista del Valencia CF.',
    'Revisa esta ficha nutricional ya generada y corrige SOLO las ingestas que entren en conflicto con alergias, intolerancias, aversiones, contexto clínico o las recomendaciones del nutricionista.',
    ...
    '- Las alergias, intolerancias, aversiones y recomendaciones específicas de ingestas tienen prioridad absoluta sobre el menú.',
    ...
    '- Tono y persona: Asegúrate de que tanto las ingestas como las notas/indicaciones del array "notes" se dirijan siempre al jugador en segunda persona del singular ("tú") de forma cercana y directa. Corrige cualquier mención en tercera persona ("el jugador", "él/ella", "su", "sus", o su nombre propio) a la segunda persona. Por ejemplo: "Desayuno fijo según tus preferencias" (en lugar de "según las preferencias del jugador") o "Vienes de readaptación y ya estás en forma" (en lugar de "Juan viene de readaptación y ya está en forma").',
...
export async function reviewAndRepairPlanData({ jugador, contextoAdicional, menu, planData, teamConfig, recomendacionesIngestas }) {
  try {
    const repairedData = await requestAiJson(buildReviewPrompt({ jugador, contextoAdicional, menu, planData, recomendacionesIngestas }));
...
export async function generarDatosPlan({ jugador, nombre, contexto, contextoAdicional, calendario, menu, teamConfig, recomendacionesIngestas }) {
  const supabase = getSupabaseAdmin();
  const resolvedMenu = menu !== undefined ? menu : await latestMenu(supabase, jugador?.equipo_id);
  const baseData = buildBasePlanData({ jugador, nombre, contexto: contexto || 'semana_normal', contextoAdicional, menu: resolvedMenu, calendario, teamConfig });
  let generatedPlan = baseData;

  try {
    const aiData = await requestAiJson(buildPrompt({ jugador, contextoAdicional, menu: resolvedMenu, baseData, recomendacionesIngestas }));
    generatedPlan = mergeAiPlanData(baseData, aiData, teamConfig);
  } catch (firstError) {
    try {
      const aiData = await requestAiJson(buildPrompt({ jugador, contextoAdicional, menu: resolvedMenu, baseData, retry: true, recomendacionesIngestas }));
      generatedPlan = mergeAiPlanData(baseData, aiData, teamConfig);
    } catch (secondError) {
