...
export function buildDayPrompt({ jugador, dayKey, dayData, menu, preMatchConfig, contextoAdicional, recsForDay }) {
  ...
  const payload = {
    ...
    reglas_calidad_nutricional: [
      ...
      'CALCULO DE PROTEINA OBLIGATORIO: En tu proceso de razonamiento interno, calcula primero los gramos de proteina pura que aporta el alimento principal (ej: 100g de pollo = 23g P, 100g de merluza = 18g P). Si la carne/pescado no alcanza por si sola el objetivo de proteina_g de la ingesta, aumenta los gramos de carne/pescado (en futbol de elite raciones de 250g-320g son normales) para que la suma total de proteina del plato cumpla exactamente el objetivo fijado. Toda la proteina de comida y cena debe provenir directamente del plato principal, NUNCA anadas yogures ni postres lacteos.',
...
export function buildWeeklyPrompt({ jugador, baseData, menu, preMatchConfig, contextoAdicional, recomendacionesIngestas }) {
  ...
  const payload = {
    ...
    reglas_calidad_nutricional_y_variedad: [
      ...
      '11. COMIDAS CON POSTRE DE FRUTA EN CASO DE POCAS INGESTAS O ALTA CARGA DE HIDRATOS: Si el jugador tiene 2 o 3 comidas al día o la comida concentra muchos hidratos (>80g HC): NO satures el plato principal con una montaña de cereales (evita poner más de 180g de quinoa/arroz o 380g de patata). Reparte los hidratos incluyendo SIEMPRE fruta fresca de postre (plátano, manzana, uvas, kiwi o mango). Toda la proteína debe provenir íntegramente del plato principal (carne, pescado, marisco, huevos), NUNCA añadas yogures ni lácteos como postre en comida o cena. En el desayuno de día de partido, combina fruta con hidratos de asimilación fácil (pan blanco tostado, copos de avena o tortas) y proteína limpia (claras o huevos), NUNCA solo fruta cruda.'
    ],
  };
