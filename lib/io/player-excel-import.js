...
import { cleanText, toNumber, normalizeYear, buildDate as asIsoDate, parseDate, slugify, playerFullName } from '@/lib/utils';
...
const FIELD_HEADERS = {
  ...
  porcentaje_musculo: ['%Peso Muscular Lee&cols', '% Peso Muscular Lee&cols', '%Peso Muscular Lee'],
  ...
  mesomorfia: ['MESO'],
  ectomorfia: ['ECTO'],
  perimetro_brazo_contraido: ['Perímetro Brazo Contraído Derecho', 'Perimetro Brazo Contraido Derecho', 'Perímetro Brazo Contraído', 'Perimetro Brazo Contraido'],
  perimetro_brazo_relajado: ['Perímetro Brazo Relajado Derecho', 'Perimetro Brazo Relajado Derecho', 'Perímetro Brazo Relajado', 'Perimetro Brazo Relajado'],
  perimetro_antebrazo: ['Perímetro Antebrazo', 'Perimetro Antebrazo'],
  perimetro_muneca: ['Perímetro Muñeca', 'Perimetro Muneca'],
  ...
  diametro_humero: ['Máx. de DH', 'Max. de DH', 'Máx de DH', 'Max de DH'],
  diametro_femur: ['Máx. de DF', 'Max. de DF', 'Máx de DF', 'Max de DF'],
  diametro_muneca: ['Máx. de DE', 'Max. de DE', 'Máx de DE', 'Max de DE'],
  somatocarta_x: ['EJE X', 'Eje X', 'Somatocarta X'],
  somatocarta_y: ['EJE Y', 'Eje Y', 'Somatocarta Y'],
  indice_ponderal: ['IP', 'Indice Ponderal', 'Índice Ponderal'],
};
...
function normalizeHeader(value) {
  const withPct = String(value || '').replace(/%/g, 'pct ');
  return normalizeName(withPct);
}

function parseExcelDate(value, { preferDayFirst = true } = {}) {
...
function mapTypedMeasurement(row, lookup) {
  ...
  // Fallbacks for diameters from DH*10 / DF*10
  if (typed.diametro_humero === null) {
    const rawDh = toNumber(valueByHeaders(row, lookup, ['DH*10', 'DH * 10']));
    if (rawDh !== null) typed.diametro_humero = Number((rawDh / 10).toFixed(2));
  }
  if (typed.diametro_femur === null) {
    const rawDf = toNumber(valueByHeaders(row, lookup, ['DF*10', 'DF * 10']));
    if (rawDf !== null) typed.diametro_femur = Number((rawDf / 10).toFixed(2));
  }

  typed.porcentaje_grasa = typed.porcentaje_grasa_faulkner ?? typed.porcentaje_grasa_yuhasz ?? null;
  if (typed.porcentaje_grasa_faulkner === null && typed.porcentaje_grasa !== null) {
    typed.porcentaje_grasa_faulkner = typed.porcentaje_grasa;
  }

  if (typed.porcentaje_musculo === null && typed.peso_muscular !== null && typed.peso_kg !== null && typed.peso_kg > 0) {
    typed.porcentaje_musculo = Math.round((typed.peso_muscular / typed.peso_kg) * 100 * 100) / 100;
  } else if (typed.peso_muscular === null && typed.porcentaje_musculo !== null && typed.peso_kg !== null) {
    typed.peso_muscular = Math.round(typed.peso_kg * (typed.porcentaje_musculo / 100) * 100) / 100;
  }

  if (typed.peso_graso === null && typed.peso_kg !== null && typed.porcentaje_grasa !== null) {
    typed.peso_graso = Math.round(typed.peso_kg * (typed.porcentaje_grasa / 100) * 100) / 100;
  }

  if (typed.peso_magro === null && typed.peso_kg !== null && typed.porcentaje_grasa !== null) {
    typed.peso_magro = Math.round(typed.peso_kg * (1 - typed.porcentaje_grasa / 100) * 100) / 100;
  }
