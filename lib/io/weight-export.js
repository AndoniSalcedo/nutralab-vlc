...
import { exportWeightsReport } from '@/services/report';
...
export function exportWeightExcel({
  ...
  const aoa = [
    ['NUTRALAB - INFORME DE TOMA DE PESO'],
  ...
  records.forEach((r, idx) => {
    ...
    let cleanStatus = 'Sin registro';
    let detalle = 'Sin registro';

    if (hasWeight) {
      if (r.status === 'verde') {
        cleanStatus = 'Óptimo';
        detalle = 'Zona Óptima';
      } else if (r.status === 'amarillo') {
        cleanStatus = 'Precaución';
        detalle = 'Zona Precaución';
      } else if (r.status === 'rojo') {
        cleanStatus = r.diff < 0 ? 'Alerta (Pérdida)' : 'Alerta (Exceso)';
        detalle = r.diff < 0 ? 'Pérdida de peso' : 'Exceso de peso';
      }
    }
  ...
  worksheet['!cols'] = [
    ...
    { wch: 16 }, // % Grasa Objetivo
    { wch: 16 }, // Peso a 10% (kg)
    { wch: 16 }, // Peso a 9% (kg)
    { wch: 16 }, // Peso a 8% (kg)
    { wch: 15 }, // Variación (kg)
    { wch: 18 }, // Estado Semáforo
    { wch: 22 }, // Detalle Semáforo
  ];
...
export async function downloadWeightPdf({
  ...
  const blob = await exportWeightsReport({
    teamName,
    fecha,
    fechaFormatted,
    records,
