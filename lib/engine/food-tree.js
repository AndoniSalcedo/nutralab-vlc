...
const isCarb = (food) => ['cereales_y_tuberculos', 'otros_granos'].includes(food.category);
const hasTag = (tag) => (food) => food.tags?.includes(tag);
const isOtherGrainFood = (food) => food.category === 'otros_granos';
...
export const FOOD_TREE = {
  ...
  children: {
    ...
    proteina: {
      ...
      children: {
        aves: {
          ...
          children: {
            pollo: {
              ...
              foodSelector: (f) => f.category === 'carnes_y_aves' && !f.tags?.includes('carne_roja') && !f.tags?.includes('cerdo') && f.name.toLowerCase().includes('pollo'),
            },
            pavo: {
              id: 'pavo',
              label: 'Pavo (Genérico)',
              ...
              foodSelector: (f) => f.category === 'carnes_y_aves' && f.name.toLowerCase().includes('pavo'),
            },
            conejo: {
              id: 'conejo',
              label: 'Conejo',
        ...
        vacuno: {
          ...
          foodSelector: (f) => f.category === 'carnes_y_aves' && f.tags?.includes('carne_roja'),
        },
        cerdo: {
          id: 'cerdo',
          label: 'Cerdo',
          ...
          foodSelector: (f) => f.category === 'carnes_y_aves' && f.tags?.includes('cerdo'),
        },
        pescado_blanco: {
          id: 'pescado_blanco',
          ...
          foodSelector: hasTag('pescado_blanco'),
        },
        pescado_azul: {
          id: 'pescado_azul',
          ...
          defaultFood: 'Salmón',
          keywords: ['pescado', 'pescados'],
          foodSelector: hasTag('pescado_azul'),
        },
        marisco: {
          id: 'marisco',
          label: 'Marisco',
        ...
        vegetal_proteina: {
          ...
          foodSelector: (f) => {
            ...
            return name.includes('tofu') || name.includes('seit') || name.includes('seitan') || name.includes('soja texturizada');
          },
        },
      },
    },
    ...
    hidratos: {
      ...
      children: {
        pasta: {
          ...
          foodSelector: (f) => isCarb(f) && f.name.toLowerCase().includes('pasta'),
          resolve(catalog) {
        ...
        arroz: {
          ...
          foodSelector: (f) => isCarb(f) && f.name.toLowerCase().includes('arroz') && !f.name.toLowerCase().includes('tortas'),
          resolve(_catalog) {
            return 'Arroz blanco';
          },
        },
        tuberculos: {
          ...
          label: 'Tubérculos',
          defaultFood: 'Patata',
          foodSelector: (f) => isCarb(f) && ['patata', 'boniato', 'yuca', 'ñoquis'].some((k) => f.name.toLowerCase().includes(k)),
        },
        panes: {
          id: 'panes',
          ...
          foodSelector: (f) => isCarb(f) && ['pan ', 'pan de', 'tostada', 'biscote', 'pan sin gluten'].some((k) => f.name.toLowerCase().includes(k)),
          resolve(catalog) {
        ...
        otros_granos: {
          ...
          isGeneric: true,
          foodSelector: isOtherGrainFood,
        },
        legumbres: {
          id: 'legumbres',
          label: 'Legumbres',
          defaultFood: 'Lenteja',
          foodSelector: (f) => f.category === 'legumbres' && f.name.toLowerCase() !== 'soja texturizada',
        },
      },
    },
    ...
    frutas: {
      ...
      foodSelector: (f) => f.category === 'frutas',
    },
    ...
    verduras: {
      ...
      foodSelector: (f) => f.category === 'verduras_y_hortalizas',
    },
    ...
    grasas_y_lacteos: {
      ...
      children: {
        ...
        frutos_secos: {
          ...
          label: 'Frutos secos',
          defaultFood: 'Nueces',
          foodSelector: (f) => f.tags?.includes('fruto_seco') || ['almendra', 'anacardo', 'avellana', 'cacahuete', 'nuez', 'pistacho', 'piñon', 'semilla'].some((keyword) => f.name.toLowerCase().includes(keyword)),
        },
        leches: {
          id: 'leches',
          label: 'Leches (Grupo genérico)',
          isGeneric: true,
          foodSelector: (f) => f.name.toLowerCase().includes('leche'),
          resolve(catalog) {
        ...
        yogures: {
          ...
          foodSelector: (f) => ['yogur', 'kéfir', 'kefir', 'skyr'].some((keyword) => f.name.toLowerCase().includes(keyword)),
          resolve(catalog) {
...
export function buildContextualPlayerFoodTree(playerTree, { mealName = 'Comida' } = {}) {
  ...
  if (baseMetadata) {
    for (const leaf of baseMetadata.byId.values()) {
      ...
      const isConserve =
        blockedIds.has('conservas_pescado') &&
        (leaf.category === 'conservas' || normalizedName.includes('conserva'));
      const isLegume =
        blockedIds.has('legumbres') &&
        (leaf.category === 'legumbres' || ['lenteja', 'garbanzo', 'alubia', 'soja texturizada'].some((keyword) => normalizedName.includes(keyword)));

      if (normalizedName && (isConserve || isLegume)) {
 ...
 */
export function resolveGenericProteinNode(clinicalCatalog, player = null, tracker = null, _isPreMatch = false, _allowEggs = true, foodTree = null) {
  ...
  if (validBranches.length === 0) return getAvailableFoods(playerTree.children.proteina.children.aves?.children?.pollo, player, playerTree)[0] || null;
 ...
 */
export function resolveGenericCarbNode(clinicalCatalog, player = null, tracker = null, _isPreMatch = false, _isDinner = false, foodTree = null) {
  const playerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
...
export function resolveGenericFruitNode(clinicalCatalog, player = null, tracker = null, _isPreMatch = false, foodTree = null) {
  ...
  const candidateFoods = getAvailableFoods(playerTree.children.frutas, player, playerTree)
    .filter((f) => !f.toLowerCase().includes('desecada') && !f.toLowerCase().includes('congelada') && !f.toLowerCase().includes('vinagre') && !f.toLowerCase().includes('compota'));
...
export function resolveDishIngredientsForPlayer(decomposedDish, clinicalCatalog, player = null, tracker = null, foodTree = null) {
  ...
  if (decomposedDish.hidrato) {
    const resolvedHidrato = resolveNodeForPlayer(decomposedDish.hidrato, clinicalCatalog, player, tracker, playerTree);
    if (!resolvedHidrato) {
      return {
        safe: false,
