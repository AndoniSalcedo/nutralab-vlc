 ...
 */
export function buildContextualPlayerFoodTree(playerTree, { mealName = 'Comida' } = {}) {
  if (!playerTree) return playerTree;

  const normMeal = String(mealName || '').toLowerCase();
  const isMainMeal = normMeal.includes('comida') || normMeal.includes('cena');
  const isDinner = normMeal.includes('cena');
  const contextKey = `${isMainMeal ? 'main' : 'other'}:${isDinner ? 'dinner' : 'not-dinner'}`;
 ...
 */
export function resolveMealTreeForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null) {
  const basePlayerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const playerTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName });
  let tree = treeOrRaw;
  ...
  const isMain = normMeal.includes('comida') || normMeal.includes('cena');
 ...
 */
export function resolveMealTreeItemsForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null) {
  const basePlayerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const contextualTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName });
