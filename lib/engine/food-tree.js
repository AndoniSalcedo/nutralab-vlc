...
export const FOOD_TREE = {
  ...
  children: {
    ...
    hidratos: {
      ...
      children: {
        pasta: {
          ...
          foodNames: FOODS_CRUDO.filter((f) => f.category === 'cereales_y_tuberculos' && f.name.toLowerCase().includes('pasta')).map((f) => f.name),
          resolve(catalog) {
        ...
        arroz: {
          ...
          foodNames: FOODS_CRUDO.filter(
            (f) => f.category === 'cereales_y_tuberculos' && f.name.toLowerCase().includes('arroz') && !f.name.toLowerCase().includes('tortas')
          ).map((f) => f.name),
          resolve(_catalog) {
            return 'Arroz blanco';
          },
        ...
        tuberculos: {
          ...
          defaultFood: 'Patata',
          foodNames: FOODS_CRUDO.filter(
            (f) => f.category === 'cereales_y_tuberculos' && ['patata', 'boniato', 'yuca', 'ñoquis'].some((k) => f.name.toLowerCase().includes(k))
          ).map((f) => f.name),
        },
        panes: {
          id: 'panes',
          ...
          foodNames: FOODS_CRUDO.filter(
            (f) => f.category === 'cereales_y_tuberculos' && ['pan ', 'pan de', 'tostada', 'biscote', 'pan sin gluten'].some((k) => f.name.toLowerCase().includes(k))
          ).map((f) => f.name),
          resolve(catalog) {
        ...
        otros_granos: {
          ...
          defaultFood: 'Cuscús',
          foodNames: ['Cuscús', 'Quinoa', 'Fideos de arroz', 'Copos de avena', 'Copos de avena sin gluten'],
        },
        legumbres: {
          id: 'legumbres',
          label: 'Legumbres',
    ...
    frutas: {
      ...
      children: {
        alta_energia: {
          ...
          defaultFood: 'Plátano',
          foodNames: ['Plátano', 'Dátil', 'Pasta de Dátil', 'Uvas', 'Mango'],
        },
        citricas: {
          id: 'citricas',
          label: 'Frutas cítricas y digestivas',
          defaultFood: 'Naranja',
          foodNames: ['Naranja', 'Zumo de naranja natural', 'Mandarina', 'Kiwi', 'Piña pelada'],
        },
        antioxidantes_rojas: {
          id: 'antioxidantes_rojas',
          label: 'Frutos rojos y antioxidantes',
          defaultFood: 'Arándanos',
          foodNames: ['Fresas', 'Arándanos', 'Frambuesa'],
        },
        clasicas_agua: {
          id: 'clasicas_agua',
          label: 'Frutas clásicas y de agua',
          defaultFood: 'Manzana',
          foodNames: ['Manzana', 'Pera', 'Sandía', 'Melón'],
        },
      },
    },
    ...
    verduras: {
      ...
      foodNames: FOODS_CRUDO.filter((f) => f.category === 'verduras').map((f) => f.name),
      children: {
        digestivas_suaves: {
          id: 'digestivas_suaves',
          label: 'Digestivas suaves (Prepartido / Carga)',
          defaultFood: 'Calabacín',
          foodNames: ['Calabacín', 'Zanahoria', 'Pimiento', 'Pimiento verde'],
        },
        mediterraneas: {
          id: 'mediterraneas',
          label: 'Mediterráneas',
          defaultFood: 'Tomate',
          foodNames: ['Tomate', 'Tomate frito', 'Berenjena', 'Cebolla', 'Puerro'],
        },
        hojas_verdes: {
          id: 'hojas_verdes',
          label: 'Hojas verdes y ensaladas',
          defaultFood: 'Lechuga romana',
          keywords: ['ensalada', 'ensaladas'],
          foodNames: ['Espinaca', 'Rúcula', 'Lechuga romana', 'Pepino'],
        },
        cruciferas: {
          id: 'cruciferas',
          label: 'Crucíferas (Descanso / Postentreno)',
          defaultFood: 'Brócoli',
          foodNames: ['Brócoli', 'Coliflor'],
        },
        otras_verduras: {
          id: 'otras_verduras',
          label: 'Otras verduras',
          defaultFood: 'Judías verdes',
          foodNames: ['Champiñón', 'Espárragos verdes', 'Judías verdes'],
        },
      },
    },
 ...
 */
export function resolveGenericProteinNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false) {
 ...
 */
export function resolveGenericCarbNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false) {
 ...
 */
export function resolveGenericVeggieNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false) {
  const candidateFoods = isPreMatch
    ? getFoodsFromTreeNode(FOOD_TREE.children.verduras.children.digestivas_suaves)
    : getFoodsFromTreeNode(FOOD_TREE.children.verduras);
...
export function resolveGenericFruitNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false) {
  const fruitChildren = FOOD_TREE.children.frutas.children;
  const candidateFoods = isPreMatch
    ? ['Plátano', 'Manzana', 'Pera']
    : [
        ...getFoodsFromTreeNode(fruitChildren.alta_energia),
        ...getFoodsFromTreeNode(fruitChildren.citricas),
        ...getFoodsFromTreeNode(fruitChildren.antioxidantes_rojas),
        ...getFoodsFromTreeNode(fruitChildren.clasicas_agua),
...
export function resolveMealTreeForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null) {
  ...
  if (tree.isComplete) {
    ...
    const dCarb = resolveGenericCarbNode(clinicalCatalog, player, tracker, isPreMatch);
    const dProt = resolveGenericProteinNode(clinicalCatalog, player, tracker, isPreMatch);
  ...
  for (const branch of tree.branches) {
    ...
    if (branch.foodName) {
      ...
      if (hasPlayerAversion(branch.foodName, player) || (clinicalCatalog && !clinicalCatalog.foodsByNormalizedName.has(normalizeFoodName(branch.foodName)))) {
        ...
        if (foodItem) {
          ...
          } else if (foodItem.category === 'cereales_y_tuberculos') {
    ...
    if (bId === 'hidratos') {
      resolvedParts.push(resolveGenericCarbNode(clinicalCatalog, player, tracker, isPreMatch));
    } else if (bId === 'proteina') {
      resolvedParts.push(resolveGenericProteinNode(clinicalCatalog, player, tracker, isPreMatch));
...
export function getTreeHidratoOptions() {
  ...
  const carbFoods = FOODS_CRUDO.filter((f) => f.category === 'cereales_y_tuberculos');
...
export function getTreeVerduraOptions() {
  ...
  const hojasKeywords = ['espinaca', 'rúcula', 'rucula', 'lechuga', 'pepino', 'acelga'];
  const hojas = verduraFoods.filter((f) => hojasKeywords.some((k) => f.name.toLowerCase().includes(k)));
  hojas.forEach((f) => seen.add(f.name));

  const cruciferasKeywords = ['brócoli', 'brocoli', 'coliflor', 'champiñón', 'champinon', 'espárrag', 'esparrag', 'judías', 'judias', 'alcachofa', 'guisantes'];
  const cruciferas = verduraFoods.filter((f) => cruciferasKeywords.some((k) => f.name.toLowerCase().includes(k)));
  cruciferas.forEach((f) => seen.add(f.name));

  const sopas = FOODS_CRUDO.filter((f) => ['gazpacho', 'salmorejo'].includes(f.name.toLowerCase()));
  sopas.forEach((f) => seen.add(f.name));

  const hortalizas = verduraFoods.filter((f) => !seen.has(f.name));
  ...
  return [
    ...
    { group: 'Hortalizas', items: mapItems(hortalizas) },
    { group: 'Hojas verdes y Ensaladas', items: mapItems(hojas) },
    { group: 'Crucíferas y Otras Verduras', items: mapItems(cruciferas) },
    { group: 'Cremas y Sopas Frías', items: mapItems(sopas) },
  ];
}
