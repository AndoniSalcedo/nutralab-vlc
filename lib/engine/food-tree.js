...
export function getFoodCategoryBranch(foodOrName, clinicalCatalog = null) {
  ...
  if (!foodObj) {
    const rawName = typeof foodOrName === 'string'
      ? foodOrName
      : (foodOrName?.id || foodOrName?.name || foodOrName?.food?.name || foodOrName?.label || '');
    const norm = normalizeFoodName(rawName);
    foodObj = clinicalCatalog?.foodsByNormalizedName?.get(norm)
      || FLAT_NODE_INDEX.get(norm)
      || findTreeNode(rawName);
  }

  const tp = foodObj?.treePath || foodObj?.foodData?.treePath || (Array.isArray(foodOrName?.treePath) ? foodOrName.treePath : null);
  ...
  // Comprobar si el identificador es directamente el de una rama canónica (ej: 'pollo', 'arroz')
  if (!proteinBranch || !carbBranch) {
    ...
    if (!proteinBranch && ['pollo', 'pavo', 'vacuno', 'ternera', 'cerdo', 'conejo', 'pescado_blanco', 'pescado_azul', 'marisco', 'embutidos_fiambres', 'conservas_pescado', 'huevos', 'vegetal_proteina'].includes(str)) {
      proteinBranch = str === 'ternera' ? 'vacuno' : str;
    }
    if (!carbBranch && ['arroz', 'pasta', 'tuberculos', 'otros_granos', 'legumbres', 'panes', 'cereales_desayuno'].includes(str)) {
      carbBranch = str;
    }
  }
...
export function extractPlannedCarbAndProtein(treeNodeOrMeal, clinicalCatalog = null) {
  ...
  if (Array.isArray(val)) {
    ...
    if (Array.isArray(val.combinations) && val.combinations.length > 0) {
      const firstCombo = val.combinations[0];
      if (Array.isArray(firstCombo?.dishes)) {
        for (const d of firstCombo.dishes) inspectItem(d);
      }
    }
 ...
 */
export function resolveGenericProteinNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false, allowEggs = true, foodTree = null) {
  ...
  if (validBranches.length === 0) {
    const fallbackChicken = getAvailableFoods(playerTree.children.proteina.children.pollo, player, playerTree)[0];
  ...
  const carbBranches = [
    {
      ...
      foods: getAvailableFoods(carbChildren.arroz, player, playerTree),
    },
    {
      id: 'pasta',
 ...
 */
export function resolveMealTreeForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainMealParam = null) {
  ...
  for (const branch of tree.branches) {
    ...
    if (bId === 'hidratos') {
      const carb = resolveGenericCarbNode(clinicalCatalog, player, tracker, false, false, playerTree);
      if (carb) resolvedParts.push(carb);
    } else if (bId === 'proteina') {
      const prot = resolveGenericProteinNode(clinicalCatalog, player, tracker, false, true, playerTree);
      if (prot) resolvedParts.push(prot);
    } else if (bId === 'verduras' || bId === 'hojas_verdes') {
      const veg = resolveGenericVeggieNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
      if (veg) resolvedParts.push(veg);
    } else if (bId === 'frutas') {
      const fruit = resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
 ...
 */
export function resolveMealTreeItemsForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainMealParam = null) {
  const isMainMeal = checkIsMainMeal(mealName, isMainMealParam ?? treeOrRaw);
  const basePlayerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
