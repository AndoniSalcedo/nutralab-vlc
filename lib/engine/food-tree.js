...
export function hasTreePath(food, pathOrSegment) {
  const tp = food?.treePath || food?.foodData?.treePath;
    ...
    proteina: {
      ...
      children: {
        ...
        embutidos_fiambres: {
          ...
          keywords: ['embutido', 'embutidos', 'fiambre', 'fiambres', 'jamon', 'jamón', 'york', 'lomo embuchado'],
 ...
 */
export function resolveIsMain(mealName, treeOrRaw = null, isMainParam = null) {
  if (isMainParam !== null && isMainParam !== undefined) return Boolean(isMainParam);
  if (treeOrRaw && typeof treeOrRaw === 'object') {
    if (treeOrRaw.isMain !== undefined) return Boolean(treeOrRaw.isMain);
    if (treeOrRaw.value && typeof treeOrRaw.value === 'object' && treeOrRaw.value.isMain !== undefined) {
      return Boolean(treeOrRaw.value.isMain);
    }
  }
...
export function buildContextualPlayerFoodTree(
  playerTree,
  { mealName = 'Comida', isMain = null, isMainMeal: isMainMealParam = null } = {}
) {
  if (!playerTree) return playerTree;

  const normMeal = String(mealName || '').toLowerCase();
  const defaultIsMain = normMeal.includes('comida') || normMeal.includes('cena');
  const isMainMeal = (isMain !== null && isMain !== undefined)
    ? Boolean(isMain)
    : (isMainMealParam !== null && isMainMealParam !== undefined)
    ? Boolean(isMainMealParam)
    : defaultIsMain;
  const isDinner = normMeal.includes('cena');
  const contextKey = `${isMainMeal ? 'main' : 'light'}:${isDinner ? 'dinner' : 'not-dinner'}`;
...
function getAvailableFoods(node, _player, foodTree = FOOD_TREE) {
  if (!node) return [];
...
/**
 * Determina la rama de proteína y/o hidratos a la que pertenece un alimento o nodo.
 */
export function getFoodCategoryBranch(foodOrName, clinicalCatalog = null) {
  ...
  let foodObj = typeof foodOrName === 'object' && foodOrName?.treePath ? foodOrName : null;
  if (!foodObj && foodOrName?.food?.treePath) {
    foodObj = foodOrName.food;
  }
  if (!foodObj && clinicalCatalog?.foodsByNormalizedName) {
    const rawName = typeof foodOrName === 'string' ? foodOrName : (foodOrName?.name || foodOrName?.food?.name || foodOrName?.label || '');
    foodObj = clinicalCatalog.foodsByNormalizedName.get(normalizeFoodName(rawName));
  }

  const tp = foodObj?.treePath || (Array.isArray(foodOrName?.treePath) ? foodOrName.treePath : null);
  ...
  // Comprobar si el identificador es directamente el de una rama canónica
  const str = String(foodOrName?.id || (typeof foodOrName === 'string' ? foodOrName : foodOrName?.name || foodOrName?.label || '')).toLowerCase().trim();
  const normStr = normalizeFoodName(str);

  if (['pollo', 'pavo', 'vacuno', 'ternera', 'cerdo', 'conejo', 'pescado_blanco', 'pescado_azul', 'marisco'].includes(str)) {
    proteinBranch = str === 'ternera' ? 'vacuno' : str;
  }
  if (['arroz', 'pasta', 'tuberculos', 'otros_granos', 'legumbres'].includes(str)) {
    carbBranch = str;
  }

  // Fallbacks por coincidencia de nombre si no tenía treePath explícito
  if (!proteinBranch) {
    if (normStr.includes('conejo')) proteinBranch = 'conejo';
    else if (normStr.includes('pollo')) proteinBranch = 'pollo';
    else if (normStr.includes('pavo')) proteinBranch = 'pavo';
    else if (normStr.includes('ternera') || normStr.includes('vacuno') || normStr.includes('vaca') || normStr.includes('buey') || normStr.includes('entrecot')) proteinBranch = 'vacuno';
    else if (normStr.includes('cerdo') || normStr.includes('lomo') || normStr.includes('secreto') || normStr.includes('presa')) proteinBranch = 'cerdo';
    else if (['merluza', 'bacalao', 'dorada', 'lubina', 'lenguado', 'corvina', 'rodaballo', 'rape'].some(f => normStr.includes(f))) proteinBranch = 'pescado_blanco';
    else if (['salmon', 'atun', 'bonito', 'caballa', 'sardina', 'trucha', 'emperador', 'pez espada'].some(f => normStr.includes(f))) proteinBranch = 'pescado_azul';
    else if (['sepia', 'calamar', 'pulpo', 'gamba', 'langostino', 'mejillon', 'berberecho'].some(f => normStr.includes(f))) proteinBranch = 'marisco';
  }

  if (!carbBranch) {
    if (normStr.includes('arroz') && !normStr.includes('leche')) carbBranch = 'arroz';
    else if ((normStr.includes('pasta') || normStr.includes('espagueti') || normStr.includes('macarron') || normStr.includes('fideo')) && !normStr.includes('datil')) carbBranch = 'pasta';
    else if (normStr.includes('patata') || normStr.includes('boniato') || normStr.includes('yuca') || normStr.includes('batata') || normStr.includes('noca') || normStr.includes('noquis')) carbBranch = 'tuberculos';
    else if (normStr.includes('avena') || normStr.includes('quinoa') || normStr.includes('cuscus') || normStr.includes('bulgur') || normStr.includes('sarraceno') || normStr.includes('maiz')) carbBranch = 'otros_granos';
    else if (normStr.includes('lenteja') || normStr.includes('garbanzo') || normStr.includes('alubia')) carbBranch = 'legumbres';
  }
...
export function extractPlannedCarbAndProtein(treeNodeOrMeal, clinicalCatalog = null) {
  ...
  const inspectItem = (item) => {
    ...
    if (!carbBranch && cat.carbBranch) {
      ...
      carbFood = typeof item === 'string' ? item : item?.name || item?.label;
    }
    if (!proteinBranch && cat.proteinBranch) {
      proteinBranch = cat.proteinBranch;
      proteinFood = typeof item === 'string' ? item : item?.name || item?.label;
    }
  };
  ...
  if (Array.isArray(val)) {
    ...
    if (Array.isArray(val.firstDishes) && val.firstDishes.length > 0) {
      inspectItem(val.firstDishes[0]?.dish);
    }
    if (Array.isArray(val.secondDishes) && val.secondDishes.length > 0) {
      inspectItem(val.secondDishes[0]?.dish);
    }
 ...
 */
export function resolveGenericProteinNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false, _allowEggs = true, foodTree = null) {
  ...
  const proteinBranches = Object.entries(proteinChildren)
    .filter(([id]) => id !== 'embutidos_fiambres' && id !== 'conservas_pescado' && id !== 'huevos')
    .map(([id, childNode]) => ({
      id,
      label: childNode.label,
      foods: getAvailableFoods(childNode, player, playerTree),
    }));

  const validBranches = proteinBranches.filter((b) => {
    return b.foods.length > 0;
  });

  if (validBranches.length === 0) return getAvailableFoods(playerTree.children.proteina.children.pollo, player, playerTree)[0] || null;
 ...
 */
export function resolveGenericCarbNode(clinicalCatalog, player = null, tracker = null, isPreMatch = false, _isDinner = false, foodTree = null) {
  const playerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  ...
  const carbBranches = [
    {
      ...
      foods: getAvailableFoods(carbChildren.arroz, player, playerTree).filter((f) => !f.toLowerCase().includes('leche')),
    },
    {
      id: 'pasta',
      label: carbChildren.pasta?.label || 'Pasta',
      foods: getAvailableFoods(carbChildren.pasta, player, playerTree),
    },
    {
      id: 'tuberculos',
      label: carbChildren.tuberculos?.label || 'Tubérculos',
      foods: getAvailableFoods(carbChildren.tuberculos, player, playerTree),
    },
    {
      id: 'otros_granos',
      label: carbChildren.otros_granos?.label || 'Otros granos y cereales',
      foods: getAvailableFoods(carbChildren.otros_granos, player, playerTree),
    },
    {
      id: 'legumbres',
      label: carbChildren.legumbres?.label || 'Legumbres',
      foods: getAvailableFoods(carbChildren.legumbres, player, playerTree),
    },
  ];

  const validBranches = carbBranches.filter((b) => b.foods.length > 0);

  if (validBranches.length === 0) return getAvailableFoods(carbChildren.arroz, player, playerTree)[0] || null;
 ...
 */
export function resolveGenericVeggieNode(clinicalCatalog, player = null, tracker = null, _isPreMatch = false, foodTree = null) {
  const playerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const valid = getAvailableFoods(playerTree.children.verduras, player, playerTree);
 ...
 */
export function resolveGenericFruitNode(clinicalCatalog, player = null, tracker = null, _isPreMatch = false, foodTree = null) {
  const playerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const candidateFoods = getAvailableFoods(playerTree.children.frutas, player, playerTree)
    .filter((f) => {
      ...
      return !low.includes('desecada') && !low.includes('congelada') && !low.includes('vinagre') && !low.includes('compota') && !low.includes('pasta de dátil') && !low.includes('pasta de datil');
    });
...
export function resolveNodeForPlayer(nodeOrId, clinicalCatalog, player = null, tracker = null, foodTree = null) {
  ...
  if (normKey === 'proteina') return resolveGenericProteinNode(clinicalCatalog, player, tracker, false, true, playerTree);
  if (normKey === 'hidratos') return resolveGenericCarbNode(clinicalCatalog, player, tracker, false, false, playerTree);
  if (normKey === 'verduras' || normKey === 'hojas_verdes') return resolveGenericVeggieNode(clinicalCatalog, player, tracker, false, playerTree);
  if (normKey === 'frutas') return resolveGenericFruitNode(clinicalCatalog, player, tracker, false, playerTree);
  ...
  if (typeof node.resolve === 'function' && clinicalCatalog) {
    ...
    if (resolvedName && getAvailableFoods({ foodNames: [resolvedName] }, player, playerTree).length > 0) {
      return resolvedName;
    }
  }
  ...
  if (Array.isArray(node.foodNames) && node.foodNames.length > 0) {
    const validFoods = getAvailableFoods(node, player, playerTree);
  ...
  if (node.children) {
    const childList = Object.values(node.children).filter((child) => getAvailableFoods(child, player, playerTree).length > 0 || child.children);
  ...
  if (node.defaultFood) {
    if (getAvailableFoods({ foodNames: [node.defaultFood] }, player, playerTree).length > 0) {
 ...
 */
export function getCompleteMealBranches(mealName = 'Comida', _clinicalCatalog = null, _player = null, _isPreMatch = false) {
 ...
 */
export function resolveMealTreeForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainParam = null) {
  const isMain = resolveIsMain(mealName, treeOrRaw, isMainParam);
  const basePlayerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const playerTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName, isMain });
  let tree = treeOrRaw;
  ...
  if (tree.isComplete) {
    if (normMeal.includes('desayuno')) {
      const bCarb = dayIndex % 2 === 0
        ? (panApto ? `Tostadas de ${panApto.toLowerCase()}` : null)
        : (resolveNodeForPlayer('cereales_desayuno', clinicalCatalog, player, tracker, playerTree)
           || resolveNodeForPlayer('otros_granos', clinicalCatalog, player, tracker, playerTree));
      const bProt = dayIndex % 2 === 0
        ? resolveNodeForPlayer('pavo', clinicalCatalog, player, tracker, playerTree)
        : resolveNodeForPlayer('huevos', clinicalCatalog, player, tracker, playerTree);
      const bFruit = resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
      const bFat = dayIndex % 2 === 0
        ? resolveNodeForPlayer('aceites', clinicalCatalog, player, tracker, playerTree)
        : resolveNodeForPlayer('aguacate', clinicalCatalog, player, tracker, playerTree);
    ...
    if (normMeal.includes('merienda') || normMeal.includes('snack') || normMeal.includes('almuerzo')) {
      const sFruit = resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
    ...
    const dCarb = resolveGenericCarbNode(clinicalCatalog, player, tracker, false, false, playerTree);
    const dProt = resolveGenericProteinNode(clinicalCatalog, player, tracker, false, true, playerTree);
    const dVeg = resolveGenericVeggieNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
    const dFruit = resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, playerTree);
  ...
  for (const branch of tree.branches) {
    ...
    if (bId === 'hidratos') {
      resolvedParts.push(resolveGenericCarbNode(clinicalCatalog, player, tracker, false, false, playerTree));
    } else if (bId === 'proteina') {
      resolvedParts.push(resolveGenericProteinNode(clinicalCatalog, player, tracker, false, true, playerTree));
    } else if (bId === 'verduras' || bId === 'hojas_verdes') {
      resolvedParts.push(resolveGenericVeggieNode(clinicalCatalog, player, tracker, isPreMatch, playerTree));
    } else if (bId === 'frutas') {
      resolvedParts.push(resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, playerTree));
    } else if (bId === 'panes') {
      if (panApto) resolvedParts.push(`Tostadas de ${panApto.toLowerCase()}`);
    } else if (bId === 'huevos') {
  ...
  const hasScalableComponent = resolvedParts.some((part) => {
  ...
  if (isMain && !hasFat && hasScalableComponent) {
 ...
 */
export function resolveMealTreeItemsForDay(treeOrRaw, dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainParam = null) {
  const isMain = resolveIsMain(mealName, treeOrRaw, isMainParam);
  const basePlayerTree = getPlayerFoodTree(clinicalCatalog, foodTree);
  const contextualTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName, isMain });
  ...
  return resolveMealTextToFoodItems(resolvedText, clinicalCatalog, contextualTree);
}
