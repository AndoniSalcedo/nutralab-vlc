...
export function resolveMealTreeForDay(treeOrRaw, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainMealParam = null) {
  let resolvedMealName = mealName;
  let resolvedClinicalCatalog = clinicalCatalog;
  let resolvedPlayer = player;
  let resolvedIsPreMatch = isPreMatch;
  let resolvedTracker = tracker;
  let resolvedFoodTree = foodTree;
  let resolvedIsMainMealParam = isMainMealParam;

  if (typeof mealName === 'number') {
    resolvedMealName = typeof clinicalCatalog === 'string' ? clinicalCatalog : 'Comida';
    resolvedClinicalCatalog = player;
    resolvedPlayer = isPreMatch;
    resolvedIsPreMatch = tracker || false;
    resolvedTracker = foodTree;
    resolvedFoodTree = isMainMealParam;
    resolvedIsMainMealParam = arguments[8] ?? null;
  }

  const isMainMeal = checkIsMainMeal(resolvedMealName, resolvedIsMainMealParam ?? treeOrRaw);
  const basePlayerTree = getPlayerFoodTree(resolvedClinicalCatalog, resolvedFoodTree);
  const playerTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName: resolvedMealName, isMainMeal });
  let tree = treeOrRaw;
  if (tree && typeof tree === 'object' && tree.tree && Array.isArray(tree.tree.branches)) {
    tree = tree.tree;
  }
  if (!tree || typeof tree === 'string' || !tree.branches) {
    tree = buildMealTree(treeOrRaw || '', resolvedMealName, resolvedClinicalCatalog, resolvedPlayer, resolvedIsPreMatch);
  }

  if (!tree.isValid) {
    return `[Fallo en árbol de ${resolvedMealName}: ${tree.error || 'Configuración no válida'}]`;
  }
  ...
  if (tree.isAlternativeSet) {
    return `[Alternativas de ${resolvedMealName} pendientes de seleccionar]`;
  }

  const normMeal = String(resolvedMealName || '').toLowerCase();
  const panApto = resolveNodeForPlayer('panes', resolvedClinicalCatalog, resolvedPlayer, null, playerTree);

  // Si es Árbol Completo, armar el plato según el tipo de toma
  if (tree.isComplete) {
    if (normMeal.includes('desayuno')) {
      return resolveGenericBreakfastMeal(resolvedClinicalCatalog, resolvedPlayer, resolvedTracker, resolvedIsPreMatch, playerTree);
    }

    if (normMeal.includes('merienda') || normMeal.includes('snack') || normMeal.includes('almuerzo')) {
      const sFruit = resolveGenericFruitNode(resolvedClinicalCatalog, resolvedTracker, resolvedIsPreMatch, playerTree);
      const sDairy = resolveNodeForPlayer('yogures', resolvedClinicalCatalog, resolvedPlayer, resolvedTracker, playerTree);
      const sNut = resolveNodeForPlayer('frutos_secos', resolvedClinicalCatalog, resolvedPlayer, resolvedTracker, playerTree);
      return [sFruit, sDairy, sNut].filter(Boolean).join(', ');
    }

    // Comida o Cena completa
    const dCarb = resolveGenericCarbNode(resolvedClinicalCatalog, resolvedTracker, false, playerTree);
    const dProt = resolveGenericProteinNode(resolvedClinicalCatalog, resolvedTracker, false, true, playerTree);
    const dVeg = resolveGenericVeggieNode(resolvedClinicalCatalog, resolvedTracker, resolvedIsPreMatch, playerTree);
    const dFruit = resolveGenericFruitNode(resolvedClinicalCatalog, resolvedTracker, resolvedIsPreMatch, playerTree);
    const dFat = resolveNodeForPlayer('aceites', resolvedClinicalCatalog, resolvedPlayer, resolvedTracker, playerTree);
...
export function resolveMealTreeItemsForDay(treeOrRaw, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainMealParam = null) {
  ...
  const isMainMeal = checkIsMainMeal(resolvedMealName, resolvedIsMainMealParam ?? treeOrRaw);
  const basePlayerTree = getPlayerFoodTree(resolvedClinicalCatalog, resolvedFoodTree);
  const contextualTree = buildContextualPlayerFoodTree(basePlayerTree, { mealName: resolvedMealName, isMainMeal });
  const resolvedText = resolveMealTreeForDay(
    treeOrRaw,
    resolvedMealName,
    resolvedClinicalCatalog,
    resolvedPlayer,
    resolvedIsPreMatch,
    resolvedTracker,
    basePlayerTree,
  ...
  return resolveMealTextToFoodItems(resolvedText, resolvedClinicalCatalog, contextualTree);
}
