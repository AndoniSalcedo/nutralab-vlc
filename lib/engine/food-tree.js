...
export const FOOD_TREE = {
  ...
  children: {
    ...
    frutas: {
      ...
      foodSelector: (f) => hasTreePath(f, 'frutas') && !hasTreePath(f, 'desecadas'),
    },
...
export function buildMealTree(text, mealName = 'Comida', clinicalCatalog = null) {
  ...
  // 1. Árboles ya construidos, alternativas o completo explícito
  ...
  // 2. Extraer alimentos candidatos (desde objeto tipado o desde texto plano)
  const typedItems = [];
  const rawText = String(text?.raw || (typeof text === 'string' ? text : '') || '').trim();

  if (text && typeof text === 'object') {
    const addTyped = (field, cat) => {
      const arr = Array.isArray(field) ? field : (field ? [field] : []);
      for (const it of arr) {
        if (it && it !== 'Sin grasa añadida') {
          typedItems.push({ item: String(it).trim(), category: cat });
        }
      }
    };
    addTyped(text.hidrato, 'hidratos');
    addTyped(text.proteina, 'proteina');
    addTyped(text.verdura, 'verduras');
    addTyped(text.fruta, 'frutas');
    addTyped(text.lacteo, 'lacteos');
    addTyped(text.grasa, 'grasas');
  } else if (rawText) {
    for (const part of rawText.split(/[,+/]/).map((s) => s.trim()).filter(Boolean)) {
      typedItems.push({ item: part, category: null });
    }
  }

  // 3. Único bucle para resolver ramas contra el árbol taxonómico
  const branches = [];
  const seenIds = new Set();

  for (const { item, category: knownCategory } of typedItems) {
    const node = findTreeNode(item, lookupTree) || findTreeNode(item, FOOD_TREE);
    if (node) {
      ...
      if (!seenIds.has(nodeId)) {
        ...
        branches.push({
          ...
          label: node.label,
          isGeneric: isGen,
          category: knownCategory || node.category || node.id,
          foodName: (!isGen && node.isLeaf) ? (node.foodNames?.[0] || node.label) : null,
          defaultFood: node.defaultFood || (node.foodNames ? node.foodNames[0] : null),
        });
      }
    } else {
      const isSingleRawText = typeof text === 'string' && typedItems.length === 1;
      if (!isSingleRawText) {
        ...
        if (!seenIds.has(normKey)) {
          ...
          branches.push({
            ...
            label: item,
            isGeneric: false,
            category: knownCategory || 'especifico',
            foodName: item,
          });
        }
      }
  ...
  // 4. Si no hay ramas reconocidas (o venía vacío/libre) -> Árbol Completo con rotación variada
  const isStillComplete = branches.length === 0;
  const finalBranches = isStillComplete ? getCompleteMealBranches(mealName) : branches;
  ...
  return {
    ...
    conflicts: [],
    isValid: true,
    label: (typeof text === 'object' && text.label) || (isStillComplete ? `Árbol completo (${finalBranches.map((b) => b.label).join(' + ')})` : finalBranches.map((b) => b.label).join(' + ')),
  };
}
