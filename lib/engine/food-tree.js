...
export function resolveGenericBreakfastMeal(clinicalCatalog, player = null, tracker = null, isPreMatch = false, playerTree = null) {
  ...
  const availableBreads = getAvailableFoods(breadNode, player, tree);

  const cerealNode = tree.children?.hidratos?.children?.cereales_desayuno;
  const availableCereals = getAvailableFoods(cerealNode, player, tree);

  const embutidosNode = tree.children?.proteina?.children?.embutidos_fiambres;
  const availableEmbutidos = getAvailableFoods(embutidosNode, player, tree);

  const huevosNode = tree.children?.proteina?.children?.huevos;
  const availableHuevos = getAvailableFoods(huevosNode, player, tree);

  const yoguresNode = tree.children?.grasas_y_lacteos?.children?.yogures;
  const availableYogures = getAvailableFoods(yoguresNode, player, tree);

  const quesosNode = tree.children?.grasas_y_lacteos?.children?.quesos;
  const availableQuesos = getAvailableFoods(quesosNode, player, tree);

  const conservasNode = tree.children?.proteina?.children?.conservas_pescado;
  const availableConservas = getAvailableFoods(conservasNode, player, tree);

  const frutosSecosNode = tree.children?.grasas_y_lacteos?.children?.frutos_secos;
  const availableFrutosSecos = getAvailableFoods(frutosSecosNode, player, tree);
  ...
  if (availableBreads.length > 0 && availableEmbutidos.length > 0) {
    styles.push({
      ...
      getFoods: () => {
        ...
        const fruit = resolveGenericFruitNode(clinicalCatalog, player, tracker, isPreMatch, tree);
  ...
  // 3. Tostadas con Queso Fresco o Conservas de Pescado
 ...
 */
export function resolveMealTreeForDay(treeOrRaw, _dayIndex = 0, mealName = 'Comida', clinicalCatalog = null, player = null, isPreMatch = false, tracker = null, foodTree = null, isMainMealParam = null) {
