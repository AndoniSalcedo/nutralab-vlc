 ...
 * categorías entre sí.
 */
function resolveMealAlternativesTree(mealData, clinicalCatalog, player, tracker, mealName, playerFoodTree) {
  const alternatives = Array.isArray(mealData?.alternativas) ? mealData.alternativas : [];
  if (alternatives.length === 0) return null;

  const contextualTree = buildContextualPlayerFoodTree(playerFoodTree, { mealName });
  const isMainMeal = /comida|cena/i.test(String(mealName || ''));
...
export async function generateDeterministicWeeklyPlan({
  ...
  for (let dayIdx = 0; dayIdx < daysOfWeek.length; dayIdx++) {
    ...
    for (const meal of resolvedMeals) {
      const { ing, rawResult, rawText, isAlert, fixedOnly } = meal;
