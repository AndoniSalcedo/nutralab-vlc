  ...
  if (Array.isArray(mealData.platos_desglosados) && mealData.platos_desglosados.length > 0) {
    ...
    if (safeDishes.length > 0) {
      ...
      return {
        ...
        dessertDishes,
        firstDishes: safeDishes.filter((sd) => sd.dish.curso === 'primero'),
        secondDishes: safeDishes.filter((sd) => sd.dish.curso === 'segundo'),
      };
    }
  }
...
function resolveMenuOptionsTree(menuOptions, tracker, clinicalCatalog = null, isDinner = false) {
  ...
  if (Array.isArray(menuOptions.combinations) && menuOptions.combinations.length > 0) {
    ...
    if (targetProteinToAvoid) {
      const nonCollidingProt = pool.filter((c) => !c.proteinBranch || c.proteinBranch !== targetProteinToAvoid);
    ...
    if (targetCarbToAvoid) {
      const nonCollidingCarb = pool.filter((c) => !c.carbBranch || c.carbBranch !== targetCarbToAvoid);
  ...
  } else if (Array.isArray(menuOptions.firstDishes) && Array.isArray(menuOptions.secondDishes)) {
    ...
    selectedDishes = [firstDish, secondDish].filter(Boolean);
  }
