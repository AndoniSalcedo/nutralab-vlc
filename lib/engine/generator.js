...
export function calculateMealBudgets(day, options = {}) {
  ...
  const hasAlmuerzo = mealNames.some((n) => n.toLowerCase().includes('almuerzo'));
  const hasComida = mealNames.some((n) => n.toLowerCase().includes('comida'));
...
function chooseMenuDish(dishes, tracker, record = true, shouldAvoid = null) {
  ...
  const nonRecent = candidatePool.filter((item) => !tracker?.isDishRecent(item.dish.nombre));
  const pool = nonRecent.length > 0 ? nonRecent : candidatePool;
  const chosen = pool[Math.floor(Math.random() * pool.length)];
  if (record && chosen?.dish?.nombre) tracker?.recordDish(chosen.dish.nombre);
...
function resolveMenuOptionsTree(menuOptions, tracker, clinicalCatalog = null, isDinner = false) {
  ...
  const firstDish = chooseMenuDish(
    menuOptions.firstDishes,
    tracker,
    true,
    targetCarbToAvoid
      ? (sd) => {
          const branchInfo = extractPlannedCarbAndProtein(sd.dish, clinicalCatalog);
          return branchInfo.carbBranch && branchInfo.carbBranch === targetCarbToAvoid;
        }
      : null
  );

  const targetProteinToAvoid = plannedDinner?.proteinBranch || todayLunch?.proteinBranch;
  const secondDish = chooseMenuDish(
    menuOptions.secondDishes,
    tracker,
    true,
    targetProteinToAvoid
      ? (sd) => {
          const branchInfo = extractPlannedCarbAndProtein(sd.dish, clinicalCatalog);
          return branchInfo.proteinBranch && branchInfo.proteinBranch === targetProteinToAvoid;
        }
      : null
  );
...
function calculateMacroTotals(items) {
  if (!Array.isArray(items) || items.length === 0) return { kcal: 0, p: 0, hc: 0, g: 0 };
  return items.reduce((totals, item) => {
    const grams = Number(item?.grams);
    const food = item?.food;
    if (!Number.isFinite(grams) || !food) return totals;
    return {
      kcal: totals.kcal + (grams / 100) * Number(food.kcal || 0),
      p: totals.p + (grams / 100) * Number(food.pro || 0),
      hc: totals.hc + (grams / 100) * Number(food.cho || 0),
      g: totals.g + (grams / 100) * Number(food.fat || 0),
    };
  }, { kcal: 0, p: 0, hc: 0, g: 0 });
}

function prepareFixedMealItems(rawResult) {
  if (!Array.isArray(rawResult)) {
    return {
      rawResult,
      fixedComponents: 0,
      fixedMacros: { kcal: 0, p: 0, hc: 0, g: 0 },
      fixedOnly: false,
    };
  }
  ...
  const hasPrimaryProtein = prepared.some((item) => getFoodRootBranch(item?.food) === 'proteina');
  const hasPrimaryCarb = prepared.some((item) => getFoodRootBranch(item?.food) === 'hidratos');
  const fixedOnly = prepared.length > 0 && hasBranchData && (!hasPrimaryProtein || !hasPrimaryCarb);
  const fixedPrepared = fixedOnly
    ? prepared.map((item) => {
        if (Number.isFinite(Number(item?.grams))) return { ...item, isFixedComponent: true };
        const minGrams = Number(item?.food?.minGrams);
        return Number.isFinite(minGrams)
          ? { ...item, grams: minGrams, isFixedComponent: true }
 ...
 * categorías entre sí.
 */
function resolveMealAlternativesTree(mealData, clinicalCatalog, player, tracker, mealName, playerFoodTree, isMainParam = null) {
  const alternatives = Array.isArray(mealData?.alternativas) ? mealData.alternativas : [];
  if (alternatives.length === 0) return null;

  const isMain = resolveIsMain(mealName, mealData, isMainParam);
  const contextualTree = buildContextualPlayerFoodTree(playerFoodTree, { mealName, isMain });
  const isMainMeal = isMain;
  ...
  alternatives.forEach((alternative, index) => {
    ...
    const itemNames = [...resolved.items];
    const hasExplicitFat = Boolean(alternative.grasa);
    if (isMainMeal && !hasExplicitFat) itemNames.push('AOVE');

    const items = resolveMealTextToFoodItems(itemNames.join(', '), clinicalCatalog, contextualTree);
...
export async function generateDeterministicWeeklyPlan({
  ...
  const wrapMealValue = (value, isPreMatch = false) => {
    const isMain = value && typeof value === 'object' && value.isMain !== undefined
      ? Boolean(value.isMain)
      : null;
    return {
  ...
  for (let dayIdx = 0; dayIdx < daysOfWeek.length; dayIdx++) {
    ...
    const cenaIngesta = dayData.ingestas.find((i) => i.nombre.toLowerCase().includes('cena'));
    if (cenaIngesta) {
      const cenaTree = weeklyTrees[dayKey]?.[cenaIngesta.nombre.toLowerCase().trim()];
    ...
    for (const ing of dayData.ingestas) {
      ...
      const isMain = tree?.isMain !== null && tree?.isMain !== undefined
        ? Boolean(tree.isMain)
        : resolveIsMain(mealName, tree?.value);
      ...
      if (!rawResult && tree?.kind !== 'fixed' && tree?.kind !== 'meal-alternatives') {
      ...
      const prepared = prepareFixedMealItems(rawResult);
      const isPostFixed = tree?.kind === 'fixed';
      if (isPostFixed) {
        prepared.fixedMacros = { kcal: 80, p: 20, hc: 0, g: 0 };
        prepared.fixedComponents = 0;
        prepared.fixedOnly = true;
      }

      const rawText = Array.isArray(prepared.rawResult)
        ? prepared.rawResult.map((item) => item.name || item.food?.name).filter(Boolean).join(', ')
        : prepared.rawResult;
    ...
    for (const meal of resolvedMeals) {
      const { ing, rawResult, rawText, isAlert, fixedOnly, isMainMeal: mealIsMain } = meal;
      const normMeal = ing.nombre.toLowerCase().trim();
      ...
      const macrosReales = calibrated?.macrosReales || null;
