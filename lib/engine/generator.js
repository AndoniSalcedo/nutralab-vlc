...
import { calibrateMeal, findFoodInCatalog } from './calculator.js';
 ...
 */
export function selectBuffetMealDishes(mealData, clinicalCatalog, player = null, tracker = null, isDinner = false) {
  ...
  if (Array.isArray(mealData.platos_desglosados) && mealData.platos_desglosados.length > 0) {
    ...
    for (const d of mealData.platos_desglosados) {
      if (hasPlayerAversion(d, player)) continue;
      const res = resolveDishIngredientsForPlayer(d, clinicalCatalog, player, tracker);
    ...
    if (safeDishes.length > 0) {
      const allCarbDishes = safeDishes.filter((sd) => sd.dish.hidrato);
      const nonLegumeCarbDishes = allCarbDishes.filter((sd) => {
        const hidrato = String(sd.dish.hidrato).toLowerCase();
        return !['legumbre', 'lenteja', 'garbanzo', 'alubia'].some((term) => hidrato.includes(term));
      });
      const carbDishes = isDinner && nonLegumeCarbDishes.length > 0
        ? nonLegumeCarbDishes
        : allCarbDishes;
      const proteinDishes = safeDishes.filter((sd) => sd.dish.proteina?.length > 0 && !carbDishes.includes(sd));
      const dessertDishes = safeDishes.filter((sd) => (sd.dish.fruta?.length > 0 || sd.dish.lacteo?.length > 0) && !carbDishes.includes(sd) && !proteinDishes.includes(sd));

      // 1. Elección de plato de hidrato: 1 entre 1 si solo hay uno, o aleatorio no reciente si hay varios
      let carbDish = null;
      if (carbDishes.length === 1) {
        carbDish = carbDishes[0];
      } else if (carbDishes.length > 1) {
        const nonRecent = carbDishes.filter((cd) => !tracker?.isDishRecent(cd.dish.nombre));
        const pool = nonRecent.length > 0 ? nonRecent : carbDishes;
        carbDish = pool[Math.floor(Math.random() * pool.length)];
      }

      if (carbDish) {
        tracker?.recordDish(carbDish.dish.nombre);
      }

      // 2. Elección de plato de proteína: 1 entre 1 si solo hay uno, o aleatorio no reciente si hay varios
      let proteinDish = null;
      if (proteinDishes.length === 1) {
        proteinDish = proteinDishes[0];
      } else if (proteinDishes.length > 1) {
        const nonRecent = proteinDishes.filter((pd) => !tracker?.isDishRecent(pd.dish.nombre));
        const pool = nonRecent.length > 0 ? nonRecent : proteinDishes;
        proteinDish = pool[Math.floor(Math.random() * pool.length)];
      }

      if (proteinDish) {
        tracker?.recordDish(proteinDish.dish.nombre);
      }

      // 3. Postre si hay disponible
      let dessertDish = null;
      if (dessertDishes.length === 1) {
        dessertDish = dessertDishes[0];
      } else if (dessertDishes.length > 1) {
        dessertDish = dessertDishes[Math.floor(Math.random() * dessertDishes.length)];
      }
  ...
  const parsed = parseFoodsFromText(combinedText, clinicalCatalog, player);
  if (parsed.length > 0) {
  ...
  for (let dayIdx = 0; dayIdx < daysOfWeek.length; dayIdx++) {
    ...
    for (const ing of dayData.ingestas) {
      ...
      // 2. Cena de carga pre-partido (24h previas)
        ...
        rawStr = resolveMealTreeForDay(previousMatchPauta, dayIdx, mealName, clinicalCatalog, jugador, true, tracker);
      }
      // 3. Día de partido (protocolo pre-partido)
        ...
        if (pautaMatch) {
          rawStr = resolveMealTreeForDay(pautaMatch, dayIdx, mealName, clinicalCatalog, jugador, true, tracker);
        }
      }
      if (!rawStr) {
        ...
        if (normMeal.includes('comida') || normMeal.includes('almuerzo') || normMeal.includes('cena')) {
          ...
          const buffetOption = selectBuffetMealDishes(mealService, clinicalCatalog, jugador, tracker, !isLunch);

          if (buffetOption) {
            rawStr = buffetOption;
          } else {
            const pautaDefecto = jugador?.recomendaciones_defecto?.[mealName] || jugador?.recomendaciones_defecto?.[mealName.toLowerCase()];
            rawStr = resolveMealTreeForDay(pautaDefecto, dayIdx, mealName, clinicalCatalog, jugador, false, tracker);
          }
        } else {
