...
/**
 * Caché en memoria para coincidencias de similitud léxica
 */
...
export function parseMealItem(itemText, options = {}) {
  ...
  // Extraer gramos o mililitros si están especificados
  const gMatch = clean.match(/(\d+(?:\.\d+)?)\s*(?:g|gr|gramos|ml)/i);
  let grams = gMatch ? parseFloat(gMatch[1]) : null;

  // Extraer unidades (ej. "Ensure Nutrición Entera 1 unidad")
  let hasUnits = false;
  let unitCount = 1;
  const uMatch = clean.match(/(\d+)\s*(?:unidades|unidad)/i);
  if (uMatch) {
  ...
  // Detección de huevos y claras: ej: "1 Huevo", "2 Huevos y 6 Claras", "7 Claras"
  ...
  // Batido post-entreno o recovery
  const cleanLow = clean.toLowerCase();
  if (cleanLow.includes('recovery') && grams === null) {
    grams = 100;
  }

  // Limpiar el nombre para la búsqueda en el catálogo
  let nameOnly = clean
    .replace(/\b\d+(?:\.\d+)?\s*(?:g|gr|gramos|ml)\b/gi, '')
    .replace(/\b\d+\s*(?:unidades|unidad)\b/gi, '')
    .replace(/^[\s,+-]+|[\s,+-]+$/g, '')
    .replace(/\s+/g, ' ')
    .trim();

  // Búsqueda en catálogo oficial
  const food = findFoodInCatalog(nameOnly, options);

  // Si no se han especificado gramos, asignar ración base de partida
  if (grams === null) {
    const cat = food?.category || '';
    if (cat === 'frutas') {
      grams = nameOnly.toLowerCase().includes('platano') ? 120 : food.minGrams;
    } else if (cat === 'verduras_y_hortalizas' || cat === 'verduras') {
      grams = food.minGrams;
    } else if (food?.minGrams) {
      grams = food.minGrams;
    }
  }

  // Aplicar límites minGrams y maxGrams del catálogo
  if (food) {
...
export async function calibrateMeal(mealDetailStr, targetBudget, options = {}) {
  ...
  // Separar ingredientes por delimitadores estándar (+, ,, /)
  ...
  if (targetBudget.hc >= 80 && !hasFruit) {
    ...
    if (!selectedFruit) {
      selectedFruit = findFoodInCatalog('Plátano', options);
    }

    if (selectedFruit) {
      const isBanana = selectedFruit.name.toLowerCase().includes('platano');
      const dessertGrams = isBanana ? 120 : selectedFruit.minGrams;
      const dessertFruit = parseMealItem(`${selectedFruit.name} ${dessertGrams}g`, options);
  ...
  parsedItems.forEach((it, idx) => {
    ...
    if (!isFruit && (['cereales_y_tuberculos', 'otros_granos'].includes(cat) || (it.food?.cho || 0) > 15)) {
        ...
        if (excessCho <= availableFruitCho && fruitItem) {
          ...
          if (remExcessAfterFruit < 20) {
            ...
            const existingCereal = parsedItems.find((it, idx) => {
              if (idx === primaryCarbIndex) return false;
              return ['cereales_y_tuberculos', 'otros_granos'].includes(it.food?.category) && !it.name.toLowerCase().includes('patata') && !it.name.toLowerCase().includes('boniato');
            });
