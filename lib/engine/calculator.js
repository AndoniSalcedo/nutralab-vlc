...
export async function calibrateMeal(mealDetailStr, targetBudget, options = {}) {
  ...
  if (targetBudget.hc >= 80 && !hasFruit) {
    ...
    const resolvedFruitName = resolveNodeForPlayer(
      'frutas',
  ...
  parsedItems.forEach((it, idx) => {
    ...
    const isWrap = hasTreePath(it.food, 'panes') && (
      it.name.toLowerCase().includes('fajita') ||
      it.name.toLowerCase().includes('tortilla de trigo')
    );
    const isDishCarb = hasTreePath(it.food, 'arroz') ||
      ...
      isWrap;
    const isBread = hasTreePath(it.food, 'panes') && !isWrap;
  ...
  if (isMainMeal && primaryCarbIndex !== -1) {
    ...
    if (breadIdx !== -1) {
      ...
      const minBreadCho = 30 * (Number(breadItem.food.cho) / 100);
  ...
  if (primaryCarbIndex !== -1) {
    ...
    const minP = isMainMeal && isDishCarb
      ? Math.max(carbItem.food.minGrams, 75)
      : carbItem.food.minGrams;
    const isBreadCarb = hasTreePath(carbItem.food, 'panes');
    const maxP = isBreadCarb
      ? Math.max(carbItem.food.maxGrams, options.isMatchDay ? 150 : 120)
      : carbItem.food.maxGrams;
    ...
    if (!fruitItem && isMainMeal) {
      ...
      if (dessertFruit) {
        const baseFruitGrams = Math.min(120, Math.max(100, Number(dessertFruit.minGrams) || 100));
        fruitItem = {
    ...
    if (isTuber) {
      ...
      if (rawCarbGrams <= maxP) {
        ...
        const currentFruitGrams = fruitItem ? (fruitItem.grams || 100) : 100;
        ...
        if (excessCho <= availableFruitCho && fruitCanAdjust) {
          ...
          if (remExcessAfterFruit < 20) {
            ...
            if (existingCereal) {
              ...
              if (!cerealFood) {
                cerealFood = getCatalogFood('Arroz blanco', options) || getCatalogFood('Arroz', options);
              }
              if (cerealFood) {
      ...
      if (rawCarbGrams < minP) {
        ...
        if (excessCho <= availableFruitCho && fruitCanAdjust) {
          ...
          if (breadItem) {
            ...
            const maxBread = Math.max(
              breadItem.food.maxGrams,
              options.isMatchDay ? 150 : 120
            );
    ...
    if (proteinItemIndices.length === 1) {
      ...
      if (isEggItem(singleItem)) {
        const maxPro = singleItem.food?.maxGrams
          ? (Number(singleItem.food.maxGrams) / 100) * Number(singleItem.food.pro || 13)
          : neededTotalPro;
      ...
      if (neededTotalPro < minTotalRequired) {
        ...
        if (isEggPrimary) {
          const maxPro = primaryItem.food?.maxGrams
            ? (Number(primaryItem.food.maxGrams) / 100) * Number(primaryItem.food.pro || 13)
            : neededTotalPro;
          const targetPro = Math.min(neededTotalPro, maxPro);
          const eggCalc = calculateEggAndClaras(targetPro, 2);
  ...
  parsedItems.forEach(it => {
    ...
    const isDishCarb = hasTreePath(it.food, 'arroz') ||
      ...
      hasTreePath(it.food, 'otros_granos') ||
      hasTreePath(it.food, 'legumbres');
    const minG = isMainMeal && isDishCarb ? Math.max(it.food.minGrams, 75) : it.food.minGrams;
    const isBread = hasTreePath(it.food, 'panes');
    const maxG = isBread
      ? Math.max(it.food.maxGrams, options.isMatchDay ? 150 : 120)
      : it.food.maxGrams;
