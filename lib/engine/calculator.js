...
export function parseMealItem(itemText, options = {}) {
  ...
  if (grams === null) {
    if (food?.defaultGrams) {
      ...
      if (cat === 'frutas') {
        grams = nameOnly.toLowerCase().includes('platano') ? 120 : 150;
...
export async function calibrateMeal(mealDetailStr, targetBudget, options = {}) {
  ...
  if (targetBudget.hc >= 80 && !hasFruit) {
    ...
    if (selectedFruit) {
      ...
      const defaultG = isBanana ? 120 : (selectedFruit.defaultGrams || 150);
      const dessertFruit = parseMealItem(`${selectedFruit.name} ${defaultG}g`, options);
