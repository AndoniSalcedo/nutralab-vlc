...
export async function calibrateMeal(mealDetailStr, targetBudget, options = {}) {
  ...
  if (targetBudget.hc >= 80 && !hasFruit) {
    ...
    if (selectedFruit) {
      const isBanana = hasTreePath(selectedFruit, 'platano') || selectedFruit.name.toLowerCase().includes('platano');
  ...
  const isMainMeal = options.isMainMeal !== undefined
    ? Boolean(options.isMainMeal)
    : options.isMain !== undefined
    ? Boolean(options.isMain)
    : options.mealName && (
  ...
  if (isMainMeal && primaryCarbIndex === -1) {
    const isCena = Boolean(options.mealName && options.mealName.toLowerCase().includes('cena'));
    let chosenCarb = null;
