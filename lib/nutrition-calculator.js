...
const QUICK_ALIASES = {
  ...
  'atun al natural': 'Atún natural - conserva natural',
  'atún al natural': 'Atún natural - conserva natural',
  'atun al natural en lata': 'Atún natural - conserva natural',
  'atún al natural en lata': 'Atún natural - conserva natural',
  'queso fresco batido': 'Queso fresco batido / desnatado',
  'queso fresco batido 0%': 'Queso fresco batido / desnatado',
  ...
  'yogur proteico': 'Yogur proteico natural (Hacendado/Mercadona)',
  'yogur proteico 0%': 'Yogur proteico natural (Hacendado/Mercadona)',
...
export async function resolveUnknownFoodWithAI(foodName) {
  ...
  try {
    ...
    const response = await client.messages.create({
      model: env.AI_PLAN_MODEL || 'claude-3-7-sonnet-20250219',
      max_tokens: 200,
      temperature: 0.1,
      messages: [
        {
 ...
 */
export async function calibrateMeal(mealDetailStr, targetBudget) {
  ...
  if (primaryCarbIndex !== -1) {
    ...
    if (foodNameLow.includes('patata') || foodNameLow.includes('boniato') || foodNameLow.includes('batata')) {
      exactCarbGrams = Math.max(150, Math.min(650, exactCarbGrams));
      ...
      // Granos secos: arroz, pasta, quinoa, cuscús
      exactCarbGrams = Math.max(50, Math.min(260, exactCarbGrams));
    }
  ...
  // Reconstruir la descripción con formato limpio y elegante (redondeo a 5g garantizado)
  const resultParts = parsedItems.map(it => {
    const low = it.name.toLowerCase();
    if (low.includes('huevo entero') && it.hasUnits) {
      return `Huevo entero ${it.unitCount} unidades (${roundToFive(it.grams)}g)`;
    }
    if (low.includes('ensure')) {
      return 'Ensure Nutrición Entera 1 unidad';
    }
    // Nombre del alimento con mayúscula inicial y gramos redondeados al 5
    const displayName = it.name.charAt(0).toUpperCase() + it.name.slice(1);
