...
/**
 * Devuelve la URL del avatar de un jugador si existe avatar_size, avatar_url o base64.
 * Incluye timestamp de updated_at para evitar que el navegador cachee fotos antiguas al cambiarlas.
 */
export function getPlayerAvatarUrl(player) {
  if (!player) return undefined;
  if (player.avatar_url) return player.avatar_url;
  if (player.avatar_size) {
    const timestamp = player.updated_at ? `&t=${encodeURIComponent(player.updated_at)}` : '';
    return `/api/players/avatar?id=${player.id}${timestamp}`;
  }
  if (typeof player.avatar === 'string' && player.avatar.startsWith('data:')) {
...
export async function compressAvatar(file, {
  ...
  const attempt = async (w, h, quality) => {
    canvas.width = w;
    canvas.height = h;
    // Rellenar con blanco para que fondos transparentes aparezcan en blanco y no en negro
